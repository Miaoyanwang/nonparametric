...
%DIF DEL Nonparametric_matrixââzhangComment.tex   Thu Jan 21 23:03:41 2021
%DIF ADD Nonparametric_matrix.tex                     Fri Jan  8 22:56:34 2021
\newcommand{\blind}{1}
...
\if1\blind
{   \date{}
  \title{\bf Nonparametric \DIFdelbegin \DIFdel{trace regression }\DIFdelend \DIFaddbegin \DIFadd{learning with matrix-valued predictors }\DIFaddend in high dimensions}
...
First, we develop a new nonparametric regression paradigm -- learning reduction~\citep{vapnik2013nature} -- that solves regression through a \DIFdelbegin \DIFdel{sequence }\DIFdelend \DIFaddbegin \DIFadd{series }\DIFaddend of classifications. Figure~\ref{fig:diagram}a illustrates our learning reduction approaches. We convert a hard \DIFdelbegin \emph{%DIFDELCMD < {\rc %%%
\DIFdel{What}%DIFDELCMD < }%%%
} %DIFAUXCMD
\DIFdel{problem }\DIFdelend \DIFaddbegin \DIFadd{problem }\DIFaddend \emph{``what is the value of \DIFdelbegin %DIFDELCMD < {\rc %%%
\DIFdel{$p(\mX)$}%DIFDELCMD < }%%%
\DIFdel{''}\DIFdelend \DIFaddbegin \DIFadd{$\mu(\mX)$}\DIFaddend ?\DIFaddbegin \DIFadd{''}\DIFaddend } to an easier \emph{\DIFdelbegin %DIFDELCMD < {\rc %%%
\DIFdel{Is}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{where}\DIFaddend } problem \emph{``\DIFdelbegin %DIFDELCMD < {\rc %%%
\DIFdel{Is $p(\mX)$ smaller than $\pi$}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{where does $\mu(\mX)$ fall below a given value}\DIFaddend ?''}\DIFdelbegin %DIFDELCMD < {\rc %%%
\DIFdel{for a given $\pi\in(0,1)$}%DIFDELCMD < }%%%
\DIFdelend . The latter is related to classification which is statistically easier to address. Most existing nonparametric regression such as nearest neighbors, local polynomials, regularing kernels, etc, extract information from data by exploiting the neighborhood structure in the domain space~\citep{wasserman2006all}. In contrast, our reduction method focuses on the neighborhood structure in the range space, and uses a \DIFdelbegin \DIFdel{sequence }\DIFdelend \DIFaddbegin \DIFadd{series }\DIFaddend of classifications to bridge the range space and the domain space. It is worth noting that the former is a 1-dimensional bounded random variable whereas the latter is a $d_1d_2$-dimensional random variable. The shifted focus to the simpler range space brings both statistical and computational benefits; see Sections~\ref{sec:idea} and~\ref{sec:estimation} for details. 
...
\caption{(a) Our learning reduction approach (solid line) to the three problems of interest. The classical plug-in approaches are depicted in dashed line.  (b) Schematic diagram for nonparametric function estimation via level set aggregations (i.e,\ a \DIFdelbeginFL \DIFdelFL{sequence }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{series }\DIFaddendFL of weighted classifications). Figure (b) is modified from~\cite{gibou2018review}. }\label{fig:diagram}
\end{figure}
...
\[
\DIFdelbegin %DIFDELCMD < {\rc %%%
\DIFdel{p}\DIFdelend \DIFaddbegin \DIFadd{\mathbb{E}}\DIFaddend (\DIFaddbegin \DIFadd{y|}\DIFaddend \mX)\DIFdelbegin %DIFDELCMD < }%%%
\DIFdelend =g(\langle \mB,\mX\rangle), \ \text{with a known function }g(\cdot)\colon \mathbb{R}\to \mathbb{R},
\]
...
These methods in some sense address the first and \DIFdelbegin \DIFdel{(}\DIFdelend part of the second \DIFdelbegin \DIFdel{) challenge }\DIFdelend \DIFaddbegin \DIFadd{challenges }\DIFaddend mentioned in the earlier paragraph; they use the conventional wisdom of exploiting low-dimensional structure in the input data in order to achieve accurate prediction. 
...
\item {\it Matrix classification}: Classification is the problem of predicting the label $y\in \{-1,1\}$ to which the new matrix $\mX$ belongs. A prediction rule (also called a classifier) decides that $y=1$ if $\mX\in S$ and \DIFdelbegin \DIFdel{$y={\rc -1}$ }\DIFdelend \DIFaddbegin \DIFadd{$y=0$ }\DIFaddend if $\mX \not\in S$, where $S$ is a Borel subset of $\tX$. We formulate the classification problem as choosing a classifier $S\in \tS$, from a given set of candidate classifiers $\tS$, that minimizes the expected classification error 
...
\bayesS(1/2)=\{\mX\in \tX \colon \DIFdelbegin %DIFDELCMD < {\rc %%%
\DIFdel{p}\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend (\mX)\DIFdelbegin %DIFDELCMD < }%%%
\DIFdelend \geq 1/2\}.
\end{equation}
Note that the minimizer of~\eqref{eq:classloss} is non-unique, because arbitrary prediction rules are allowed on the boundaries \DIFdelbegin \DIFdel{$\partial S_{\text{bayes}}(1/2) = \{\mX\in \tX\colon {\rc p(\mX)}=1/2\}$}\DIFdelend \DIFaddbegin \DIFadd{$\partial S_{\text{bayes}}(1/2) = \{\mX\in \tX\colon \mu(\mX)=1/2\}$}\DIFaddend . Without loss of generality, we will use~\eqref{eq:bayes} as the canonical form of the Bayes classifier. 
...
\item {\it Level set estimation}: The problem of level set estimation generalizes the classification task. For a given $\pi\in(0, 1)$, the target $\pi$-level set of the conditional probability function \DIFdelbegin \DIFdel{$\rc{ p(\mX)}$ }\DIFdelend \DIFaddbegin \DIFadd{$\mu(\mX)$ }\DIFaddend is defined as
\begin{equation}\label{eq:level}
\bayesS(\pi) = \{\mX\in\tX \colon \DIFdelbegin %DIFDELCMD < {\rc %%%
\DIFdel{p}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend (\mX)\geq \pi\}.
\end{equation}
...
\end{equation}
where $y\in\{-1,1\}$ is distributed based on \DIFdelbegin \DIFdel{$\mathbb{P}(y=1|\mX)={\rc p}(\mX)$}\DIFdelend \DIFaddbegin \DIFadd{$\mathbb{P}(y=1|\mX)=\mu(\mX)$}\DIFaddend , and we define $w_{\pi}(y)=1-\pi$ or $\pi$ depending on $y=1$ or $-1$; that is, $w_\pi(\cdot)$ assigns unequal weights to the two labels. In light of~\eqref{eq:classloss} and~\eqref{eq:risklevel}, the level set problem extends the usual classification from equal weight $\pi=1/2$ to general weight $\pi \in(0,1)$. %Accurate level set estimation plays an important role in applications of geographical elevation maps, imaging contour detection, and motion tracking. 
...
\item {\it Nonparametric binary regression}: \DIFdelbegin %DIFDELCMD < {\rc %%%
\DIFdel{For a binary response, the}%DIFDELCMD < }  %%%
\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend problem of nonparametric regression is to estimate the conditional \DIFdelbegin %DIFDELCMD < {\rc %%%
\DIFdel{probability function $p(\mX)=\mathbb{P}(Y=1|\mX)$}%DIFDELCMD < }%%%
\DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{mean $\mathbb{E}(y|\mX)$ as a multivariable function in the predictor space. In our contexts, the nonparametric regression is equivalent to estimating the conditional probability $\mu(\mX)=\mathbb{P}(y=1|\mX)=\frac{1}{2}(\mathbb{E}(y|\mX)+1)$. }\DIFaddend Throughout the paper we will focus on \DIFdelbegin \DIFdel{${\rc p(\mX)}$ }\DIFdelend \DIFaddbegin \DIFadd{$\mu(\mX)$ }\DIFaddend and refer to it as the regression function. 
The function \DIFdelbegin \DIFdel{${\rc p}(\mX)$}\DIFdelend \DIFaddbegin \DIFadd{$\mu(\mX)$}\DIFaddend , is the global minimizer, among all measurable functions $f\colon \tX\to[0,1]$, of the expected squared error,
\begin{equation}\label{eq:regression}
R_{\text{reg}}(f)=\mathbb{E}\left[\DIFdelbegin %DIFDELCMD < {\rc %%%
\DIFdel{\frac{1}{2}(}\DIFdelend y+\DIFdelbegin \DIFdel{1)-f}\DIFdelend \DIFaddbegin \DIFadd{1-2f}\DIFaddend (\mX)\DIFdelbegin %DIFDELCMD < }%%%
\DIFdelend \right]^2,
\end{equation}
where $R_{\text{reg}}(f)$ is also known as regression risk. Our final goal is the function estimation:

{\bf Question 3.} How to learn the binary regression function \DIFdelbegin \DIFdel{${\rc p}(\mX)$ }\DIFdelend \DIFaddbegin \DIFadd{$\mu(\mX)$ }\DIFaddend in the high-dimensional matrix space?
\end{enumerate}
...
\begin{figure}
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=15cm]{demo_method.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=15cm]{demo_method2.pdf}
\DIFaddendFL \caption{Level-set approaches to matrix binary nonparametric regression. We use weighted classification to find the level-set in the matrix space, and then estimate the target regression function via aggregation. \DIFdelbeginFL %DIFDELCMD < {\rc %%%
\DIFdelFL{(Should the two slices in the middle be swapped?)}%DIFDELCMD < } %%%
\DIFdelendFL }\label{fig:method}
\end{figure}
...
In this section we describe the oracle procedure of estimating regression function \DIFdelbegin \DIFdel{${\rc p}(\mX)$ }\DIFdelend \DIFaddbegin \DIFadd{$\mu(\mX)$ }\DIFaddend when the true distribution $\mathbb{P}_{\mX,y}$ is known. This simplified situation leads to a cleaner characterization with deterministic risk functions in~\eqref{eq:classloss}, \eqref{eq:risklevel}, and \eqref{eq:regression}. The finite sample estimation will be presented in Section~\ref{sec:estimation}, in which we address the general case with unknown distribution $\mathbb{P}_{\mX,y}$, and \DIFdelbegin \DIFdel{the only information is through }\DIFdelend \DIFaddbegin \DIFadd{all the information is based on }\DIFaddend empirical (stochastic) risk estimated from the training set $(\mX_i,y_i)_{i=1}^n$. 


\subsection{Level set approaches to nonparametric binary regression}\label{sec:bridge}
Figure~\ref{fig:method} illustrates the main idea of our approaches. We use a \DIFdelbegin \DIFdel{sequence }\DIFdelend \DIFaddbegin \DIFadd{series }\DIFaddend of weighted classifications to find the level sets in the matrix space, and then estimate the regression function \DIFdelbegin \DIFdel{${\rc p}(\mX)$ }\DIFdelend \DIFaddbegin \DIFadd{$\mu(\mX)={1\over 2}(\mathbb{E}(y|\mX)+1)$ }\DIFaddend via level set aggregation. Our building block is to use level sets to estimate regression function $p(\cdot)$ through classifications. The level set approach bridges the two sides of a same coin -- characteristic (set indicator) functions in functional analysis and weighted classifications in statistical learning. 

Specifically, let $\mu(\cdot)\colon \tX \to [0,1]$ be the target regression function of interest, and \DIFdelbegin \DIFdel{$\bayesS(\pi)=\{\mX\in \tX\colon {\rc p}(\mX)\geq \pi\}$ }\DIFdelend \DIFaddbegin \DIFadd{$\bayesS(\pi)=\{\mX\in \tX\colon \mu(\mX)\geq \pi\}$ }\DIFaddend be the associated $\pi$-level set. Let $\Pi=\{{1\over H}, {2\over H}, \ldots, {H-1\over H} \}$ be a sequence of evenly spaced points in $[0,1]$, where $H\in\mathbb{N}_{+}$ is the resolution parameter. We introduce an $H$-step function $\bar \mu(\cdot)\colon \tX \to [0,1]$ by
\begin{equation}\label{eq:stepfunction}
\bar \DIFdelbegin %DIFDELCMD < {\rc %%%
\DIFdel{p}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend (\mX)= {1\over 2H}  \sum_{\pi \in \Pi} \sign (\mX\in \bar S(\pi))+{1\over 2}, \quad \text{for all }\mX\in\tX\subset \mathbb{R}^{d_1\times d_2},
\end{equation}
...
In order to address the accuracy \DIFdelbegin \DIFdel{between }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend $\bar \mu(\mX)$ \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend $\mu(\mX)$, we establish the recovery guarantee of level sets $S(\pi)$ from optimization~\eqref{eq:constrained}. The Bayes classifier $\bayesS(\pi)$ minimizes the weighted classification risk $R_\pi(S)$; the inverse, however, may not be true because of possible multiple global minimizers of $R_\pi(S)$. The uniqueness and stability around $\bayesS(\pi)$ turns out to play a key role in the accurate estimation of \DIFdelbegin \DIFdel{${\rc p}(\mX)$}\DIFdelend \DIFaddbegin \DIFadd{$\mu(\mX)$}\DIFaddend . 

We introduce the following notion to characterize the behavior of the regression function near the level set boundaries \DIFdelbegin \DIFdel{$\partial S_{\text{bayes}}(\pi) = \{ {\rc p}(\mX)= \pi\}$}\DIFdelend \DIFaddbegin \DIFadd{$\partial S_{\text{bayes}}(\pi) = \{ \mu(\mX)= \pi\}$}\DIFaddend . The condition essentially quantifies the uniqueness of level sets recovery from weighted classification.

Some additional notation is needed. We call a level $\pi\in[0,1]$ a mass point if the level set boundary $\partial \bayesS(\pi)$ has non-zero measures under $\mathbb{P}_{\mX}$. Let \DIFdelbegin \DIFdel{$\tN=\{\pi\in[0,1] \colon \mathbb{P}_{\mX}\left[{\rc p}(\mX)=\pi\right]\neq 0\}$ }\DIFdelend \DIFaddbegin \DIFadd{$\tN=\{\pi\in[0,1] \colon \mathbb{P}_{\mX}\left[\mu(\mX)=\pi\right]\neq 0\}$ }\DIFaddend denote the collection of mass points in \DIFdelbegin \DIFdel{${\rc p}(\mX)$}\DIFdelend \DIFaddbegin \DIFadd{$\mu(\mX)$}\DIFaddend . Assume $|\tN|\leq c<\infty$ for some constant $c>0$. 

\begin{defn} [$\alpha$-regularity]\label{ass:decboundary} Fix $\pi \notin \tN$. A function \DIFdelbegin \DIFdel{${\rc p}(\mX)$ }\DIFdelend \DIFaddbegin \DIFadd{$\mu(\mX)$ }\DIFaddend is called $(\alpha,\pi)$-locally regular, if there exist constants $C=C(\pi)>0, \alpha=\alpha(\pi)\in[0,1]$, such that,
\begin{equation}\label{eq:mass}
\sup\DIFdelbegin \DIFdel{_{0\leq t<\rho(\pi, \tN)}{\mathbb{P}_{\mX}(|{\rc p}(\mX)-\pi|\leq t)\over t^{\alpha/(1-\alpha)}}}\DIFdelend \DIFaddbegin \DIFadd{_{0\leq t<\rho(\pi, \tN)}{\mathbb{P}_{\mX}(|\mu(\mX)-\pi|\leq t)\over t^{\alpha/(1-\alpha)}}}\DIFaddend \leq C,
\end{equation}
...
\DIFdelbegin \DIFdel{there exists a constant $c>0$ such that
}\DIFdelend \begin{align}\label{eq:identity}
...
Our identifiability characterization generalizes the earlier results for single level set estimation~\citep{singh2009adaptive,xu2020class}. Inspection of the proof shows that the set estimation is more difficult at levels where the point mass concentrates (small $\alpha$), as intuition would suggest. Consider a simple case when the entries of matrix $\mX$ are i.i.d.\ drawn from $\text{Uniform}[-1,1]$; that is, $\mathbb{P}_{\mX}$ is the Lebesque measure on $[0,1]^{d_1d_2}$. Then, the best rate $\alpha\to 1$ corresponds to a clear separation with no point mass at the boundary, whereas the worst rate $\alpha\to 0$ corresponds to a heavy mass of \DIFdelbegin \DIFdel{${\rc p}(\mX)$ }\DIFdelend \DIFaddbegin \DIFadd{$\mu(\mX)$ }\DIFaddend near the boundary. A typical intermediate case is $\alpha=1/2$ when \DIFdelbegin \DIFdel{${\rc p}(\mX)$ }\DIFdelend \DIFaddbegin \DIFadd{$\mu(\mX)$ }\DIFaddend is a locally bilipschitz function (see Proposition~\ref{prop:polynomial} in Appendix).

Now we reach the main result of this section by putting together the proposal~\eqref{eq:stepfunction},~\eqref{eq:constrained} and Proposition~\ref{thm:identifiability}. Define the regression excess risk by \DIFdelbegin \DIFdel{$R_{\textup{reg}}(\bar {\rc p}) - R_{\textup{reg}}({\rc p})$}\DIFdelend \DIFaddbegin \DIFadd{$R_{\textup{reg}}(\bar \mu) - R_{\textup{reg}}(\mu)$}\DIFaddend .
The following result bounds the regression excess risk using classification excess risk.

\begin{thm}[Nonparametric regression via weighted classifications]\label{thm:twobounds} Suppose that the regression function \DIFdelbegin \DIFdel{${\rc p}(\mX)$ }\DIFdelend \DIFaddbegin \DIFadd{$\mu(\mX)$ }\DIFaddend is $\alpha$-globally regular with $\alpha\in[0,1]$. Let \DIFdelbegin \DIFdel{$\bar {\rc p}(\mX)$ }\DIFdelend \DIFaddbegin \DIFadd{$\bar \mu(\mX)$ }\DIFaddend the step function~\eqref{eq:stepfunction} constructed from weighted classifiers. Then, the regression excess risk is \DIFdelbegin %DIFDELCMD < {\rc %%%
\DIFdel{bounded }%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{bounded }\DIFaddend by the classification excess risk; i.e.,
\begin{align}\label{eq:approximation}
R_{\textup{reg}}(\bar \mu) - R_{\textup{reg}}(\mu)&\leq 4\mathbb{E}_{\mX}\left|  \DIFdelbegin %DIFDELCMD < {\rc %%%
\DIFdel{p}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend (\mX)-\bar \DIFdelbegin %DIFDELCMD < {\rc %%%
\DIFdel{p}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend (\mX)\right|\notag\\
&\DIFdelbegin \DIFdel{\leq }\DIFdelend \DIFaddbegin \lesssim \DIFaddend {\DIFdelbegin \DIFdel{2}\DIFdelend \DIFaddbegin \DIFadd{1}\DIFaddend \over H}+ \DIFdelbegin \DIFdel{\frac{4}{H}}\DIFdelend \DIFaddbegin \DIFadd{\frac{1}{H}}\DIFaddend \sum\DIFdelbegin \DIFdel{_{\pi\in\Pi}}\DIFdelend \DIFaddbegin \DIFadd{_{\pi\in\Pi\setminus\tN}}\DIFaddend \left\{d_\pi^{\alpha}(\bayesS(\pi),\bar S(\pi))+ {1\over\rho(\pi, \tN)} d_\pi(\bayesS(\pi),\bar S(\pi))\right\},
\end{align}
for all resolution parameter $H=|\Pi|\in \mathbb{N}_{+}$. 

\end{thm}

Theorem~\ref{thm:twobounds} shows the key role of $\bar \mu(\mX)$ in bridging regression and classification. The results suggest that the estimation of $\mu(\mX)$ can be reduced to estimation of $\bar \mu(\mX)$, or equivalently, to a \DIFdelbegin \DIFdel{sequence }\DIFdelend \DIFaddbegin \DIFadd{series }\DIFaddend of weighted classifications $(\bar S(\pi))_{\pi\in\Pi}$. The regression excess risk bound~\eqref{eq:approximation} has two terms. The first term is the bias due to the step function approximation to the regression function. The second term is the classification excess risk of recovering $\bayesS(\pi)$ from optimization~\eqref{eq:constrained}. In the case of unknown population distribution $\mathbb{P}_{\mX,y}$, the second term should be plugged in using the empirical risk, which results in a variance-bias error due to finite sample size (see Section~\ref{sec:estimation}). 
...
Combining formulations~\eqref{eq:stepfunction},~\eqref{eq:optimization} and~\eqref{eq:class} yields our (oracle procedure of ) ``learning \DIFdelbegin \DIFdel{deduction}\DIFdelend \DIFaddbegin \DIFadd{reduction}\DIFaddend '' approach to nonparametric matrix regression.
...
We develop the nonparametric matrix regression in a similar way with little modification. The nonparametric regression~\eqref{eq:stepfunctionsample} estimates $\pi$-classifiers at a \DIFdelbegin \DIFdel{sequence }\DIFdelend \DIFaddbegin \DIFadd{series }\DIFaddend of weights $\pi \in \{{1\over H},\ldots, {H-1\over H}\}$. In principal, one can optimize all classifiers jointly subject to \DIFdelbegin \DIFdel{nestedness }\DIFdelend \DIFaddbegin \DIFadd{nested }\DIFaddend constraints among sequential level sets. However, this strategy would lead to increased computational burden, and the gain in accuracy is often little with moderate sample size. We choose to use parallel processing to obtain $\pi$-classifiers separately to speed up the computation. The procedure is summarized in Algorithm \ref{alg:regest}. The software for both matrix classification and regression will be available at CRAN.
...
d_{\Delta}(\bayesS(\pi),\hat S(\pi))\lesssim t_n^{\alpha/(2-\alpha)}+ {1\over \rho\DIFdelbegin \DIFdel{^2}\DIFdelend (\pi,\tN)}t_n+a^\alpha_n, \quad \text{for all levels }\pi \notin \tN, 
\end{equation}
...
We set $\Phi_i = $ empirical CDF of $z_i$ for $i=1,2$; $\Phi = $ CDF of standard normal; $g(z)=(1+\exp(z))^{-1}$; matrix dimension $d=20,30,\ldots,60$; and training sample size $n=100, 150, \ldots, 400$. The construction of \DIFdelbegin \DIFdel{$p$ }\DIFdelend \DIFaddbegin \DIFadd{$\mu$ }\DIFaddend amounts to a high nonlinearity from $\mX$ to $\mu(\mX)$. Unlike parametric methods, the functional form of \DIFdelbegin \DIFdel{$p$ }\DIFdelend \DIFaddbegin \DIFadd{$\mu$ }\DIFaddend is set unknown to the algorithm. 
...
The third experiment investigates similar aspects as before but to the task of matrix regression. We set the \DIFdelbegin \DIFdel{regularing }\DIFdelend \DIFaddbegin \DIFadd{resolution }\DIFaddend parameter $H=20$ and aggregated the multiple level-sets as in our proposal~\eqref{eq:stepfunctionsample}. Figure~\ref{fig:logistic}d shows the regression error measured by $L$-1 risk, $\mathbb{E}|\hat \mu (\mX)- \mu(\mX)|$, evaluated on test data. Again, we see that the regression error decays polynomially in sample size. Note that our matrix-valued feature has ambient dimension $30\times 30=900$ whereas the sample size is on the order of hundreds. This scenario of high feature dimension low sample size is prevalent in brain network analysis. Nevertheless, our nonparametric method consistently learns the function \DIFdelbegin \DIFdel{$p$ }\DIFdelend \DIFaddbegin \DIFadd{$\mu$ }\DIFaddend from limited data without the need to specify a priori functional form.
...
We provide illustrate examples to show the outputs returned by {\bf \small NonMAR}. Figures~\ref{fig:compare2}a and c plot the top edges selected by \NonparaM based on the moving averages of feature weights \DIFdelbegin \DIFdel{$(\hat \mB_\pi)_{\pi\in[0,1]}$ with  a window size $\Delta \pi = 0.2$}\DIFdelend \DIFaddbegin \DIFadd{$(\hat \mB_\pi)_{\pi\in\Pi}$ with  $\Pi = \{0.2,\ldots,0.8\}$}\DIFaddend . The selected region agrees well with the ground truth (Figures~\ref{fig:compare}a and c). We also investigate the relationship between edge connectivity for individual $i$ and the estimated response probability $\hat \pi_i$ (Figures~\ref{fig:compare2}b and d). The trajectory of the edge connectivity accurately resembles the ground truth function in each subregion. The results demonstrate that our method is able to recover the right ``sorting'' of individuals with respect to the response probability on a continuous spectrum. 
...
%DIFDELCMD < y\sim \text{Ber}(\pi),\ y\perp \mX|\pi, \\
%DIFDELCMD < \mX=\mB_0+\entry{\mX_{pq}}, \ \text{where\ } \mX_{pq}\stackrel{\text{indep.}}{\sim}\tN(g_{s}(\pi), \sigma^2),
%DIFDELCMD < \end{cases}%%%
...
\end{cases}\DIFaddend 
\]
where \DIFdelbegin \DIFdel{$s=s(p,q)$ is the index such that the }\DIFdelend \DIFaddbegin \DIFadd{$ \mX_{pq}\stackrel{\text{indep.}}{\sim}\tN(g_{s}(\pi), \sigma^2)$ if  the }\DIFaddend position $(p,q)$ falls in the support of $\mB_s$ \DIFaddbegin \DIFadd{and $ \mX_{pq}\stackrel{\text{indep.}}{\sim}\tN(0,\sigma^2)$  otherwise}\DIFaddend .  Recall that $\mu(\mX)=\mathbb{E}(y|\mX)$. In the noiseless case $\sigma = 0$, there exists a sequence of functions $(f_\pi)_{\pi\in(0,1)}$ with $f_\pi \in\tF(r,s_1,s_2)$, such that $\sign (\mu(\mX)-\pi) = \sign f_\pi(\mX)$ for all $\pi \in (0,1)$.
...
d(S,\bayesS) &\stackrel{\text{def}}{=} R(S)-R(\bayesS)\DIFdelbegin %DIFDELCMD < \notag %%%
\DIFdelend \\
...
\DIFdelbegin \DIFdel{Without loss of generality, suppose $ \rho(\pi,\tN)\leq C_2$. If $\rho(\pi,\tN)>C_2$, we can replace $\rho(\pi,\tN)$ by $C_2$ in }%DIFDELCMD < \eqref{eq:identity}%%%
\DIFdel{. With a modification of the constant $c$, }%DIFDELCMD < \eqref{eq:identity} %%%
\DIFdel{still holds.
}\DIFdelend We divide the proof into two cases: $\alpha \in (0,1)$ and $\alpha=1$. 
\begin{enumerate}[label={2.\arabic*},wide, labelwidth=!, labelindent=0pt]
...
&\geq t\left( \mathbb{P}_{\mX}\left(S\Delta S_{\text{bayes}}\right) - \DIFdelbegin \DIFdel{C_1t}\DIFdelend \DIFaddbegin \DIFadd{Ct}\DIFaddend ^{\alpha\over 1-\alpha}\right),\quad \text{for all }\DIFdelbegin \DIFdel{0\leq }\DIFdelend t\DIFdelbegin \DIFdel{< }\DIFdelend \DIFaddbegin \DIFadd{\in (0,}\DIFaddend \rho(\pi,\tN)\DIFaddbegin \DIFadd{)}\DIFaddend .
\end{align}
Combining the above inequality with the identity~\eqref{eq:excess} yields
\begin{equation}\label{eq:tail2}
d(S,\bayesS)\geq 2t\left( \mathbb{P}_{\mX}\left(S\Delta S_{\text{bayes}}\right) - \DIFdelbegin \DIFdel{C_1t}\DIFdelend \DIFaddbegin \DIFadd{Ct}\DIFaddend ^{\alpha\over 1-\alpha}\right),\quad \text{for all }t\in(0,\rho(\pi,\tN)]).
\end{equation}
We maximize the lower bound of~\eqref{eq:tail2} with respect to $t$ and obtain the optimal \DIFdelbegin \DIFdel{$0\leq t_{\text{opt}}< \rho(\pi,\tN)$,
}\DIFdelend \DIFaddbegin \DIFadd{$t_{\text{opt}}\in(0,\rho(\pi,\tN)])$,
}\DIFaddend \[
t_{\text{opt}}=\DIFdelbegin %DIFDELCMD < \begin{cases}
%DIFDELCMD < \rho(\pi,\tN), & \text{if}\quad \mathbb{P}_{\mX}\left(S\Delta S_{\text{bayes}}\right)\geq {C_1\over 1-\alpha}\rho^{\alpha\over1-\alpha}(\pi,\tN),\\
%DIFDELCMD < \left[{1-\alpha \over C_1}\mathbb{P}_{\mX}\left(S\Delta S_{\text{bayes}}\right)\right]^{1-\alpha \over \alpha}, & \text{if}\quad \mathbb{P}_{\mX}\left(S\Delta S_{\text{bayes}}\right)< {C_1\over 1-\alpha} \rho^{\alpha\over1-\alpha}(\pi,\tN).
%DIFDELCMD < \end{cases}%%%
\DIFdelend \DIFaddbegin \begin{cases}
...
\begin{align}
  d(S,\bayesS)\geq\DIFdelbegin %DIFDELCMD < \begin{cases}
%DIFDELCMD < 2\alpha \rho(\pi,\tN)\mathbb{P}_{\mX}(S\Delta \bayesS), & \text{if}\quad \mathbb{P}_{\mX}\left(S\Delta S_{\text{bayes}}\right)\geq {C_1\over 1-\alpha}\rho^{\alpha\over1-\alpha}(\pi,\tN),\\
%DIFDELCMD < 2\alpha{\left(1-\alpha\over C_1\right)}^{1-\alpha \over \alpha} \mathbb{P}^{1\over \alpha}_{\mX}\left(S\Delta S_{\text{bayes}}\right), & \text{if}\quad \mathbb{P}_{\mX}\left(S\Delta S_{\text{bayes}}\right)< {C_1\over 1-\alpha}\rho^{\alpha\over1-\alpha}(\pi,\tN).
%DIFDELCMD < \end{cases}%%%
\DIFdelend \DIFaddbegin \begin{cases}
...
\begin{align}\DIFaddbegin \label{eq:cmultiidentity}
    \DIFaddend d_\Delta(S,\bayesS) \stackrel{\text{def}}{=}\mathbb{P}\DIFdelbegin \DIFdel{_{\mX}}\DIFdelend (S\Delta\bayesS)\leq c\left(d^\alpha(S,\bayesS)+\frac{1}{\rho(\pi,\tN)}d(S,\bayesS)\right),
\end{align}

where we take \DIFdelbegin \DIFdel{$c= \max\left(\frac{1}{2\alpha},\left(C_1\over 1-\alpha\right)^{1-\alpha}\left(1\over 2\alpha\right)^\alpha\right)$}\DIFdelend \DIFaddbegin \DIFadd{$c= \max\left(\frac{1}{2\alpha},\left(C\over 1-\alpha\right)^{1-\alpha}\left(1\over 2\alpha\right)^\alpha\right)$}\DIFaddend . 
...
\DIFadd{d(S,\bayesS)\geq 2t\mathbb{P}_{\mX}(S\Delta \bayesS) = 2td_\Delta(S,\bayesS), \quad \text{for all }t\in[0,\rho(\pi,\tN)], \pi \notin\tN.
}\]\DIFaddend 
\DIFdelbegin \DIFdel{Therefore the conclusion follows by taking $t={\rho(\pi,\tN)\over 2}$ in the inequality~}%DIFDELCMD < \eqref{eq:1}%%%
\DIFdelend \DIFaddbegin \DIFadd{The inequality }\eqref{eq:cmultiidentity} \DIFadd{holds with $c={1\over 2}$ plugging $t=1/\rho(\pi,\tN)$ into the above inequality}\DIFaddend .
\end{enumerate}
\DIFaddbegin \DIFadd{Therefore,  Proposition~\ref{thm:identifiability} holds.
}\DIFaddend \end{proof}
...
&\leq  {1\over H}\sum_{\pi \in \Pi}\mathds{1}(\mX \in \bar S(\pi) \Delta \bayesS(\pi))+ {1\over 2H}\\&\leq\frac{1}{H}\sum_{\pi\in\Pi\setminus\tN}\mathds{1}(\mX \in \bar S(\pi) \Delta \bayesS(\pi))+ {1+\DIFdelbegin \DIFdel{2|}%DIFDELCMD < \tN%%%
\DIFdel{|}\DIFdelend \DIFaddbegin \DIFadd{2c}\DIFaddend \over 2H}\DIFdelbegin %DIFDELCMD < \\&%%%
\DIFdel{\leq \max_{\pi\in\Pi\setminus\tN}\mathds{1}(}%DIFDELCMD < \mX %%%
\DIFdel{\in }%DIFDELCMD < \bar %%%
\DIFdel{S(\pi) \Delta }%DIFDELCMD < \bayesS%%%
\DIFdel{(\pi))+}%DIFDELCMD < {%%%
\DIFdel{1+2C'}%DIFDELCMD < \over %%%
\DIFdel{2H}%DIFDELCMD < }%%%
\DIFdel{.
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{where the last inequality follows from the assumption that $|\tN|\leq c$.
}\DIFaddend Taking expectation with respect to $\mX$ on both sides gives
\begin{align}
\mathbb{E}|\mu(\mX)-\bar \mu(\mX)|&\leq \DIFdelbegin \DIFdel{\max_{\pi \in \Pi\setminus\tN}}\DIFdelend \DIFaddbegin \DIFadd{\frac{1}{H}\sum_{\pi\in\Pi\setminus\tN}}\DIFaddend d_\Delta(\bar S(\pi), \bayesS(\pi))+{1+\DIFdelbegin \DIFdel{2C'}\DIFdelend \DIFaddbegin \DIFadd{2c}\DIFaddend \over 2H}\\&\DIFdelbegin \DIFdel{\leq c\max_{\pi \in \Pi\setminus\tN}}\DIFdelend \DIFaddbegin \lesssim \DIFadd{\frac{1}{H}\sum_{\pi\in\Pi\setminus\tN}}\DIFaddend \left[d_\pi^\alpha(\bar S(\pi), \bayesS(\pi))+\DIFdelbegin \DIFdel{Hd}\DIFdelend \DIFaddbegin \DIFadd{\frac{1}{\rho(\pi,\tN)}d}\DIFaddend _\pi(\bar S(\pi), \bayesS(\pi))\right]+{1\DIFdelbegin \DIFdel{+2C'}\DIFdelend \over \DIFdelbegin \DIFdel{2H}\DIFdelend \DIFaddbegin \DIFadd{H}\DIFaddend },
\end{align}
where the last line comes from \DIFdelbegin \DIFdel{condition~}%DIFDELCMD < \eqref{eq:identity}%%%
\DIFdelend \DIFaddbegin \DIFadd{Proposition~\ref{thm:identifiability}}\DIFaddend . 
...
Consider the parametric model $p=g\circ f$ as in Example 1, where we assume random predictors $\mX$ with i.i.d.\ Uniform$[0,1]$ entries. Suppose the inverse link function has a constant $c_1,c_2>0$ and $\alpha\in[0,1]$ such that  $g^{-1}(\pi+t)-g^{-1}(\pi-t) \leq c_1 t^{\alpha\over 1-\alpha}$ for all \DIFdelbegin \DIFdel{$t\in(0,c_2)$}\DIFdelend \DIFaddbegin \DIFadd{$t\in[0,c_2)$}\DIFaddend . Then,
\begin{equation}\label{eq:concentration}
\mathbb{P}(|\mu(\mX)-\pi|\leq t)\leq c_3t^{\alpha\over 1-\alpha}, \quad \text{for all  }t\in\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin [\DIFaddend 0,c_2),
\end{equation}
...
\begin{enumerate}
    ...
    \begin{align}
        \int_{\{\mx:l(t)\leq \langle \mb,\mx\rangle \leq u(t)\}} d\mx  &= \int_{\{\mx:l(t)-b_{k+1}x_{k+1}\leq b_1x_1+\ldots b_kx_k\leq u(t)-b_{k+1}x_{k+1}\}} d\mx_{1:k}dx_{k+1} \\&\DIFdelbegin \DIFdel{= }\DIFdelend \DIFaddbegin \DIFadd{\leq }\DIFaddend \int_{0}^{1}\int_{l(t)-b_{k+1}x_{k+1}\leq b_1x_1+\ldots b_kx_k\leq u(t)-b_{k+1}x_{k+1}}\,d\mx_{1:k}dx_{k+1}\\&\leq \int_{0}^1 c_kt^{\alpha\over 1-\alpha}\,dx_{k+1} \\&\leq c_{k+1}t^{\alpha\over 1-\alpha}.
    \end{align}
 ...
 \end{align}
Combining  the assumption that $g^{-1}(\pi+t)-g^{-1}(\pi-t) \leq c_1t^{\alpha\over 1-\alpha}$ for all \DIFdelbegin \DIFdel{$t\in(0,c_2)$ }\DIFdelend \DIFaddbegin \DIFadd{$t\in[0,c_2)$ }\DIFaddend and \eqref{eq:mainineq} gives
 \begin{align}
     \mathbb{P}(|\mu(\mX)-\pi|\leq t/H)\leq c_3 t^{\alpha\over 1-\alpha}\DIFaddbegin \DIFadd{,  \text{ for all } t\in}[\DIFadd{0,c_2)}\DIFaddend .
 \end{align}

 In particular, when \DIFaddbegin \DIFadd{the }\DIFaddend link function is the identity or logistic link we have $\alpha = 1/2$.

 
 When $g(z) = z$, we have 
 \begin{align}
     g^{-1}(\pi+t)-g^{-1}(\pi-t) = 2t\DIFdelbegin \DIFdel{,
 }\DIFdelend \DIFaddbegin \DIFadd{.
 }\DIFaddend \end{align}
\DIFdelbegin \DIFdel{where the constant is independent on $\pi\in(0,1)$. Therefore, we have $\alpha = 1/2$ and $\beta = 1$.
}\DIFdelend 
 ...
 \DIFaddbegin \DIFadd{Therefore, we have $\alpha = 1/2$ when the link function is the identity or logistic link.
 }\DIFaddend \end{proof}
...
From Proposition~\ref{prop:equivalance},  \DIFdelbegin \DIFdel{Assumption~\ref{ass:main}(ii) implies 
}\DIFdelend \DIFaddbegin \DIFadd{$(\pi,\alpha)$-local regularity implies 
}\DIFaddend \begin{align}
\text{Var}[w(y)\ell(yf(\mX))-w(y)\ell(y\bayespif(\mX))]\DIFdelbegin \DIFdel{\leq c'}\DIFdelend \DIFaddbegin \lesssim \DIFaddend [R_{\ell,\pi}(f)-R_{\ell,\pi}(\bayespif)]^\alpha.
\end{align}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Notice $\rho(\pi,\tN)$ is a constant in the proposition because we only consider a given $\pi$.
}\DIFaddend Applying the second order condition to Theorem 3 of \cite{wang2008probability} gives that, with the choice $\lambda\leq {t_n\over 4J_n}$, 
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{We first suggest lemmas and proposition that will be used to prove Theorem~\ref{thm:regression}.
}\DIFaddend \begin{lem}[Multiple level-sets estimation]~\label{lem:main}Suppose $\mu(\mX)$ is $\alpha$-globally regular with $\alpha\in[0,1]$. Denote $t_n={ r(s_1+s_2)\log d  \over n}$ for $n\in\mathbb{N}_{+}$. Then, under the same condition and high probability specified in Theorem~\ref{thm:main}, we have
\[
...
\begin{proof}[Proof of Lemma~\ref{lem:main}] The \DIFdelbegin \DIFdel{global }\DIFdelend \DIFaddbegin \DIFadd{$\alpha$-global }\DIFaddend regularity of $\mu(\mX)$ implies that, for all $\pi \notin \tN$, the conversion inequality holds \DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{by Proposition~\ref{prop:equivalance},
}\DIFaddend \begin{equation}\label{eq:conversion2}
d_{\Delta}(\bayesS(\pi),S) \DIFdelbegin \DIFdel{\leq
}\DIFdelend \DIFaddbegin \lesssim
\DIFaddend \begin{cases}
d^\alpha_{\pi}(\bayesS(\pi),S), & \text{if }S\in \textup{I},\\
{1\over \rho(\pi, \tN)}d_{\pi}(\bayesS,S),& \text{otherwise},
\end{cases}
\end{equation}
where region  \DIFdelbegin \DIFdel{$\textup{I}=\{S \colon d_{\Delta}(\bayesS(\pi),S)\leq H^{-\alpha/(1-\alpha)}\}$.
By proposition~\ref{prop:equivalance}}\DIFdelend \DIFaddbegin \DIFadd{$\textup{I}=\{S \colon d_{\Delta}(\bayesS(\pi),S)\leq \frac{C}{1-\alpha}\rho(\pi,\tN)^{\alpha/(1-\alpha)}\}$.
In addition}\DIFaddend ,  the mean-to-variance inequality also holds \DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{by Proposition~\ref{prop:equivalance},
...
%\end{cases}
\text{Var}\left[w(y)\ell(f(\mX)) - w(y)\ell(\bayesf(\mX))\right] \DIFdelbegin \DIFdel{\leq }\DIFdelend \DIFaddbegin \lesssim \DIFaddend \left[R_{\ell,\pi}(f)-R_{\ell,\pi}(\bayesf)\right]^{\alpha}+ {1\over \rho(\pi, \tN)}\left[R_{\ell,\pi}(f)-R_{\ell,\pi}(\bayesf)\right].
\end{equation}
...
\begin{align}
d_{\Delta}(\bayesS(\pi),\hat S(\pi)) &\DIFdelbegin \DIFdel{\leq }\DIFdelend \DIFaddbegin \lesssim \DIFaddend d^\alpha_{\pi}(\bayesS(\pi),S)+{1\over \rho(\pi, \tN)}d_{\pi}(\bayesS\DIFaddbegin \DIFadd{(\pi)}\DIFaddend ,S)\\
...
\end{align}
where the last line follows from the fact that $a(b^2+b^{2-\alpha}+b+1) \leq 4 a (b^2+1)$ with $a:={t_n \over \rho^2(\pi, \tN)}$ and $b:=\rho(\pi, \tN) t_n^{(\alpha-1)/(2-\alpha)}$.  \DIFaddbegin \DIFadd{Therefore,  we obtain the first conclusion.
}\DIFaddend 


To prove the second conclusion~\eqref{eq:total}, we write 
\DIFdelbegin \[
{\DIFdel{1\over H}}\DIFdel{\sum_{\pi \in \Pi} d_{\Delta}(\bayesS(\pi),\hat S(\pi))=}{\DIFdel{1\over H}}\DIFdel{\sum_{\pi \in \Pi\cap \tN_H} d_{\Delta}(\bayesS(\pi),\hat S(\pi))+}{\DIFdel{1\over H}}\DIFdel{\sum_{\pi \in \Pi\cap \tN_H^c} d_{\Delta}(\bayesS(\pi),\hat S(\pi)),
}\]%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}\DIFadd{\label{eq:twobounds}
}{\DIFadd{1\over H}}\DIFadd{\sum_{\pi \in \Pi} d_{\Delta}(\bayesS(\pi),\hat S(\pi))=}{\DIFadd{1\over H}}\DIFadd{\sum_{\pi \in \Pi\cap \tN_H} d_{\Delta}(\bayesS(\pi),\hat S(\pi))+}{\DIFadd{1\over H}}\DIFadd{\sum_{\pi \in \Pi\cap \tN_H^c} d_{\Delta}(\bayesS(\pi),\hat S(\pi)),
}\end{align}\DIFaddend 
where $\tN_H \stackrel{\text{def}}{=}\bigcup_{\pi'\in\tN}\left(\pi'-\frac{1}{H},\pi'+\frac{1}{H}\right)$.
The first term involves only finite number of summands and thus can be \DIFdelbegin \DIFdel{absorbed in to $C'/H$}\DIFdelend \DIFaddbegin \DIFadd{bounded by $2c/H$ where $c>0$ is a constant such that $|\tN|\leq c$}\DIFaddend .  We bound the second term using the explicit forms of $\rho(\pi, \tN)$ in the sequence $\pi \in\Pi\cap \tN_H^c$,
\begin{align}
...
&\leq t_n^{\alpha/(2-\alpha)}+ \DIFdelbegin \DIFdel{2C'Ht}\DIFdelend \DIFaddbegin \DIFadd{2cHt}\DIFaddend _n,
\end{align}
where the last inequality follows from the Lemma~\ref{lem:H}.  \DIFaddbegin \DIFadd{Combining the bounds we obtained for the last two terms in }\eqref{eq:twobounds} \DIFadd{completes the second conclusion.  }\DIFaddend As seen from the calculation, the distance $\rho^2(\pi, \tN)$ grows quadratically as the level $\pi$ moves away from the mass points in $\tN$. This leads to a fast linear rate in terms of $H$, provided that there are finitly many mass points in $\tN$.
\end{proof}
...
holds with $\alpha\in(0,1]$ \DIFaddbegin \DIFadd{and $C>0$}\DIFaddend . Then, the following two properties hold for bounded functions $f\in\tF(r,s_1,s_2)$. 
\begin{enumerate}
\item Conversion inequality\DIFdelbegin \DIFdel{under hinge loss}\DIFdelend :  
...
\end{cases}
\end{equation}
where the region \DIFdelbegin \DIFdel{$\textup{I}=\{S \colon d_{\Delta}(\bayesS(\pi),S)\leq H^{-\alpha/(1-\alpha)}\}$}\DIFdelend \DIFaddbegin \DIFadd{$\textup{I}=\{S \colon d_{\Delta}(\bayesS(\pi),S)\leq \frac{C}{1-\alpha}\rho(\pi,\tN)^{\alpha/(1-\alpha)}\}$}\DIFaddend .
\item Mean-variance relationship\DIFaddbegin \DIFadd{:
}\DIFaddend \begin{equation}
\text{Var}\left[w(y)\ell(f(\mX)) - w(y)\ell(\bayesf(\mX))\right]  \lesssim 
\DIFdelbegin %DIFDELCMD < \begin{cases} 
%DIFDELCMD < \left[R_{\ell,\pi}(f)-R_{\ell,\pi}(\bayesf)\right]^{\alpha},& \text{if }\{\mX\colon f(\mX)\geq 0\}\in\textup{I},\\
%DIFDELCMD <  {1\over \rho(\pi, \tN)}\left[R_{\ell,\pi}(f)-R_{\ell,\pi}(\bayesf)\right],& \text{otherwise}.
%DIFDELCMD < \end{cases}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \left[\DIFadd{R_{\ell,\pi}(f)-R_{\ell,\pi}(}\bayesf\DIFadd{)}\right]\DIFadd{^{\alpha}+
 }{\DIFadd{1}\over \DIFadd{\rho(\pi, }\tN\DIFadd{)}}\left[\DIFadd{R_{\ell,\pi}(f)-R_{\ell,\pi}(}\bayesf\DIFadd{)}\right]\DIFadd{.
}\DIFaddend \end{equation}
\end{enumerate}
\end{prop}

\begin{proof}
Property 1 follows directly from the \DIFdelbegin \DIFdel{excess risk bound~}%DIFDELCMD < \eqref{eq:riskbound}%%%
\DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{proof in Proposition~\ref{thm:identifiability}.
For }\DIFaddend Property 2\DIFaddbegin \DIFadd{, we devide the proof into two cases: when $\ell(\cdot)$ is hinge loss and  $\psi$ loss. 
}

\begin{enumerate}[label={2.\arabic*},wide, labelwidth=!, labelindent=0pt]
\item[\DIFadd{Case 1:}] \DIFadd{When $\ell(z) = (1-z)_+$.
}

\DIFadd{Property 2 }\DIFaddend follows from Lemma~\ref{lem:hingeL1} and the boundedness of $f$. Specifically, we bound the variance using the $L$-1 distance between $f$ and $\bayespif$, 
\begin{align}
\text{Var}[w(y)\ell(yf(\mX))-w(y)\ell(y\bayespif(\mX))]&\leq  L\mathbb{E}|\ell(yf(\mX))-\ell(y\bayespif(\mX))|\\
&\leq L\mathbb{E}|f(\mX)-\bayespif(\mX)|
\DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD < &%%%
\DIFdel{\leq LCH^{\alpha}}%DIFDELCMD < \left[%%%
\DIFdel{R_{\ell,\pi}(f)-R_{\ell,\pi}(}%DIFDELCMD < \bayesf%%%
\DIFdel{)}%DIFDELCMD < \right]%%%
\DIFdel{^\alpha
}\DIFdelend \end{align}
where $L>0$ is a constant that bounds the magnitude of $f$ in the local neighborhood of $R_{\ell,\pi}(\bayespif)$ (c.f. Assumption~\ref{ass:main}), the second line comes form the Lipschitz continuity of the hinge loss\DIFdelbegin \DIFdel{, and the last line comes from }\DIFdelend \DIFaddbegin \DIFadd{.  Applying  }\DIFaddend Lemma~\ref{lem:hingeL1} \DIFdelbegin \DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{on the last inequality complete the proof.
}

\item[\DIFadd{Case 2:}] \DIFadd{When $\ell(z) = 2\min(1,(1-z)_+)$.
Notice that 
}\begin{align}
\DIFadd{\text{Var}\left[w(y)\ell(yf(\mX))-w(y)\ell(y\bayesf(\mX))\right]
\leq }& \DIFadd{\mathbb{E}|w(y)\ell(yf(\mX))-w(y)\ell(y\bayespif(\mX))|^2}\\
\DIFadd{\leq}& \DIFadd{\ L\mathbb{E}|w(y)\ell(yf(\mX))-w(y)\ell(y\bayespif(\mX))|}\\
\DIFadd{\leq }&\DIFadd{\ L\KeepStyleUnderBrace{\mathbb{E}\left| w(y)\left(1-\sign(yf(\mX))\right)- w(y)\ell(y\bayespif(\mX))\right
|}_{=:\text{(i)}}}\\&\DIFadd{+L\KeepStyleUnderBrace{\mathbb{E}\left| w(y)\ell(yf(\mX))- w(y)\left(1-\sign(yf(\mX))\right) \right|}_{=:\text{(ii)}},
}\end{align}
\DIFadd{(i) is bounded as follows
}\begin{align}
\DIFadd{\text{(i)}=\mathbb{E}\left[w(y)\left|\sign (yf(\mX))-\sign (y\bayesf(\mX))\right|\right]}&\DIFadd{\leq 2d_\Delta(\bayesS(\pi),S)}\\&\DIFadd{\lesssim  d^\alpha_{\pi}(\bayesS(\pi),S)+}{\DIFadd{1\over \rho(\pi, \tN)}}\DIFadd{d_{\pi}(\bayesS(\pi),S),
}\end{align}
\DIFadd{where the last inequality is from Property 1.
(ii) bounded as follows
}\begin{align}
    \DIFadd{\text{(ii)}
    }&\DIFadd{=\mathbb{E}\left [w(y)\ell(yf(\mX))- w(y)\left(1-\sign(yf(\mX))\right) \right]}\\
    &\DIFadd{= \mathbb{E}\left[w(y)\ell(yf(\mX))-w(y)\ell(y\bayespif(\mX))\right]+ \mathbb{E}\left[w(y)(\sign(f(\mX))-\sign(\bayespif(\mX))\right]}\\&\DIFadd{\leq }[\DIFadd{R_{\ell,\pi}(f)-R_{\ell,\pi}(\bayesf)}]\DIFadd{+2d_\Delta(\bayesS(\pi),S)}\\&\DIFadd{\lesssim  }[\DIFadd{R_{\ell,\pi}(f)-R_{\ell,\pi}(\bayesf)}]\DIFadd{+ d^\alpha_{\pi}(\bayesS(\pi),S)+}{\DIFadd{1\over \rho(\pi, \tN)}}\DIFadd{d_{\pi}(\bayesS(\pi),S),
}\end{align}
\DIFadd{where the last inequality is form Property 1.
Combining }\eqref{eq:excess-hinge} \DIFadd{and two bounds (i),(ii) completes the proof.
}\end{enumerate}
\DIFaddend \end{proof}
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \lesssim
\left[\DIFadd{R_{\ell,\pi}(f)-R_{\ell,\pi}(}\bayesf\DIFadd{)}\right]\DIFadd{^{\alpha}+
 }{\DIFadd{1}\over \DIFadd{\rho(\pi, }\tN\DIFadd{)}}\left[\DIFadd{R_{\ell,\pi}(f)-R_{\ell,\pi}(}\bayesf\DIFadd{)}\right]\DIFadd{.
}\DIFaddend \end{equation}
\DIFdelbegin \DIFdel{where the region $\textup{I}=\{S\colon d(S,\bayesS)\leq H^{-\alpha/(1-\alpha)}\}$.
}\DIFdelend \end{lem}

\begin{proof} For ease of notation, we drop the subscript $\pi$ and simply write $\bayesf$ and $R_\ell(\bayesf)$ in place of $\bayespif$ and $R_{\ell,\pi}(\bayespif)$. We also drop the random variable $\mX$ in the function expression, and simply use $f, \bayesf$, \DIFdelbegin \DIFdel{$p$}\DIFdelend \DIFaddbegin \DIFadd{$\mu$}\DIFaddend , to represent the decision function, Bayes rule, and the probability function, respectively. The meaning should be clear given the contexts. 
...
&= \int_{\mX}w(1)\DIFdelbegin \DIFdel{p}\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend (1-f)_{+}d\mathbb{P}_{\mX}+\int_{\mX}w(-1)(\DIFdelbegin \DIFdel{1-p}\DIFdelend \DIFaddbegin \DIFadd{1-\mu}\DIFaddend )(1+f)_{+}d\mathbb{P}_{\mX}\notag \\
&\quad -\int_{\mX}w(1)\DIFdelbegin \DIFdel{p}\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend (1-\bayesf)_{+}d\mathbb{P}_{\mX}-\int_{\mX}w(-1)(\DIFdelbegin \DIFdel{1-p}\DIFdelend \DIFaddbegin \DIFadd{1-\mu}\DIFaddend )(1+\bayesf)_{+}d\mathbb{P}_{\mX},
\end{align}
where $w(1)=1-\pi$ and $w(-1)=\pi$. 

In order to evaluate the integral, we divide the domain $\mX$ into four exclusive regions:
\begin{itemize}
\item Region I \DIFdelbegin \DIFdel{$= \{\mX\colon p<\pi \text{ and }f\geq -1\}$}\DIFdelend \DIFaddbegin \DIFadd{$= \{\mX\colon \mu<\pi \text{ and }f\geq -1\}$}\DIFaddend . In this region, $\bayesf=-1$, and the integrant in~\eqref{eq:function} reduces to
\begin{align}
\Phi_{\textup{I}}&:=(1-\pi)\DIFdelbegin \DIFdel{p}\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend (1-f)_{+}+\pi(\DIFdelbegin \DIFdel{1-p}\DIFdelend \DIFaddbegin \DIFadd{1-\mu}\DIFaddend )(1+f)_{+}-2(1-\pi)\DIFdelbegin \DIFdel{p}\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend \\
&\geq (1-\pi)\DIFdelbegin \DIFdel{p}\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend (1-f)+\pi(\DIFdelbegin \DIFdel{1-p}\DIFdelend \DIFaddbegin \DIFadd{1-\mu}\DIFaddend )(1+f)-2(1-\pi)\DIFdelbegin \DIFdel{p}\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend \\
&=(f+1)(\pi\DIFdelbegin \DIFdel{-p}\DIFdelend \DIFaddbegin \DIFadd{-\mu}\DIFaddend )=|f-\bayesf||\pi\DIFdelbegin \DIFdel{-p}\DIFdelend \DIFaddbegin \DIFadd{-\mu}\DIFaddend |.
\end{align}
\item Region II \DIFdelbegin \DIFdel{$= \{\mX\colon p< \pi \text{ and }f<-1\}$}\DIFdelend \DIFaddbegin \DIFadd{$= \{\mX\colon \mu< \pi \text{ and }f<-1\}$}\DIFaddend . In this region, $\bayesf=-1$, and the integrant in~\eqref{eq:function} reduces to
\[
\Phi_{\textup{II}}:=(1-\pi)\DIFdelbegin \DIFdel{p}\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend (1-f)-2(1-\pi)\DIFdelbegin \DIFdel{p}\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend =(-1-f)(\DIFdelbegin \DIFdel{p-p }\DIFdelend \DIFaddbegin \DIFadd{\mu-\mu}\DIFaddend \pi )=|f-\bayesf|(1-\pi)\DIFdelbegin \DIFdel{p}\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend .
\]
\item Region III \DIFdelbegin \DIFdel{$=\{ \mX\colon p\geq \pi \text{ and }f\leq 1\}$}\DIFdelend \DIFaddbegin \DIFadd{$=\{ \mX\colon \mu\geq \pi \text{ and }f\leq 1\}$}\DIFaddend . In this region, $\bayesf=1$, and the integrant in~\eqref{eq:function} reduces to
\begin{align}
\Phi_{\textup{III}}&:=(1-\pi)\DIFdelbegin \DIFdel{p}\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend (1-f)_{+}+\pi(\DIFdelbegin \DIFdel{1-p}\DIFdelend \DIFaddbegin \DIFadd{1-\mu}\DIFaddend )(1+f)_{+}-2\pi(\DIFdelbegin \DIFdel{1-p}\DIFdelend \DIFaddbegin \DIFadd{1-\mu}\DIFaddend )\\
&\geq (1-\pi)\DIFdelbegin \DIFdel{p}\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend (1-f)+\pi(\DIFdelbegin \DIFdel{1-p}\DIFdelend \DIFaddbegin \DIFadd{1-\mu}\DIFaddend )(1+f)-2\pi(\DIFdelbegin \DIFdel{1-p}\DIFdelend \DIFaddbegin \DIFadd{1-\mu}\DIFaddend )\\
&=(1-f)(\DIFdelbegin \DIFdel{p-}\DIFdelend \DIFaddbegin \DIFadd{\mu-}\DIFaddend \pi)=|f-\bayesf||\pi\DIFdelbegin \DIFdel{-p}\DIFdelend \DIFaddbegin \DIFadd{-\mu}\DIFaddend |
\end{align}
\item Region IV \DIFdelbegin \DIFdel{$=\{\mX\colon p\geq \pi \text{ and }f> 1\}$}\DIFdelend \DIFaddbegin \DIFadd{$=\{\mX\colon \mu\geq \pi \text{ and }f> 1\}$}\DIFaddend . In this region, $\bayesf=1$, and the integrant in~\eqref{eq:function} reduces to
\[
\Phi_{\textup{IV}}:=\pi(\DIFdelbegin \DIFdel{1-p}\DIFdelend \DIFaddbegin \DIFadd{1-\mu}\DIFaddend )(1+f)-2\pi(\DIFdelbegin \DIFdel{1-p}\DIFdelend \DIFaddbegin \DIFadd{1-\mu}\DIFaddend )=(f-1)(\pi\DIFdelbegin \DIFdel{-p}\DIFdelend \DIFaddbegin \DIFadd{-\mu}\DIFaddend \pi)=|f-\bayesf|(\DIFdelbegin \DIFdel{1-p}\DIFdelend \DIFaddbegin \DIFadd{1-\mu}\DIFaddend )\pi
\]
\end{itemize}
...
&\geq \mathbb{E}|f-\bayesf||\pi\DIFdelbegin \DIFdel{-p}\DIFdelend \DIFaddbegin \DIFadd{-\mu}\DIFaddend |\mathds{1}(|f|\leq 1)+ \mathbb{E}|f-\bayesf|(\pi\DIFdelbegin \DIFdel{-p}\DIFdelend \DIFaddbegin \DIFadd{-\mu}\DIFaddend \pi)\mathds{1}(f>1)\\
& \quad +\mathbb{E}|f-\bayesf| (\DIFdelbegin \DIFdel{p-p}\DIFdelend \DIFaddbegin \DIFadd{\mu-\mu}\DIFaddend \pi)\mathds{1}(f<-1).
\end{align}

Note that the functions \DIFdelbegin \DIFdel{$|\pi-p|, (\pi-p\pi), (p-p\pi)$ }\DIFdelend \DIFaddbegin \DIFadd{$|\pi-\mu|, (\pi-\mu\pi), (\mu-\mu\pi)$ }\DIFaddend are non-negative, $[0,1]$-valued, and satisfy~\eqref{eq:prob2} by \DIFdelbegin \DIFdel{Assumption~\ref{ass:decboundary}.  Applying }\DIFdelend \DIFaddbegin \DIFadd{$\alpha$-global regularity (local regularity follows the same argument).  Now we use Lemma~\ref{lem:product} to bound the three terms in the last equation.  To make sure that each term has the same order in }\DIFaddend Lemma~\ref{lem:product}\DIFdelbegin \DIFdel{to the above integral gives
}\begin{align*}
\DIFdel{R_\ell(f)-R_\ell(\bayesf)  }&\DIFdel{\geq CH^{-1} }[\DIFdel{\mathbb{E} |f-\bayesf|\mathds{1}(|f|\leq 1)}]\DIFdel{^{1/\alpha}+C }[\DIFdel{\mathbb{E}|f-\bayesf|\mathds{1}(f>1)}]\DIFdel{^{1/\alpha}}\\
& \DIFdel{\quad +C}[ \DIFdel{\mathbb{E}|f-\bayesf|\mathds{1}(f<-1)}]\DIFdel{^{1/\alpha}}\\
&\DIFdel{\geq CH^{-1} }[\DIFdel{\mathbb{E}|f-\bayespif|}]\DIFdel{^{1/\alpha},
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{,  we rescale the functions $|f-\bayesf|\mathds{1}(|f|\leq1),|f-\bayesf|\mathds{1}(f>1),|f-\bayesf|\mathds{1}(f<-1)$ to have expectation 1 and set upper bound of the functions as }\[\DIFadd{L = \max\left(\mnormSize{}{|f-\bayesf|\mathds{1}(|f|\leq1)},\mnormSize{}{|f-\bayesf|\mathds{1}(f>1)},\mnormSize{}{|f-\bayesf|\mathds{1}(f<-1)}\right).}\]\DIFaddend 
\DIFaddbegin \DIFadd{Then,  the last inequality is bounded either Case 1 or Case 2 from Lemma~\ref{lem:product}:
}\begin{enumerate}[label={2.\arabic*},wide, labelwidth=!, labelindent=0pt]
\item[\DIFadd{Case 1:}]
\begin{align}
\DIFadd{R_\ell(f)-R_\ell(\bayesf)  }&\DIFadd{\gtrsim }[\DIFadd{\mathbb{E} |f-\bayesf|\mathds{1}(|f|\leq 1)}]\DIFadd{^{1/\alpha}+ }[\DIFadd{\mathbb{E}|f-\bayesf|\mathds{1}(f>1)}]\DIFadd{^{1/\alpha}
+}[ \DIFadd{\mathbb{E}|f-\bayesf|\mathds{1}(f<-1)}]\DIFadd{^{1/\alpha}}\\
&\DIFadd{\geq  }[\DIFadd{\mathbb{E}|f-\bayespif|}]\DIFadd{^{1/\alpha},
}\end{align}

\DIFaddend where the last inequality uses the property that $x^{1/\alpha}+y^{1/\alpha}\geq (x+y)^{1/\alpha}$ for $x,y\geq 0$ and $\alpha\in(0,1]$.
\DIFdelbegin \DIFdel{Therefore, we conclude that
}\[
\DIFdel{\mathbb{E}|f-\bayesf|\leq C'H^{\alpha}[R_\ell(f)-R_\ell(\bayesf)]^\alpha,
}\]%DIFAUXCMD
\DIFdelend \DIFaddbegin \item[\DIFadd{Case 2:}]
\begin{align}
\DIFadd{R_\ell(f)-R_\ell(\bayesf)  }&\DIFadd{\gtrsim \rho(\pi,\tN)\left[\mathbb{E} \left(|f-\bayesf|\mathds{1}(|f|\leq 1)+|f-\bayesf|\mathds{1}(f>1)+|f-\bayesf|\mathds{1}(f<-1)\right)\right]}\\
&\DIFadd{= \rho(\pi,\tN) \mathbb{E}|f-\bayespif|.
}\end{align}\DIFaddend 
\DIFdelbegin \DIFdel{where $C'>0$ is a constant.
}\DIFdelend \DIFaddbegin \end{enumerate}
\DIFadd{Therefore, combining two cases gives
}\begin{equation}
\DIFadd{\mathbb{E}|f(\mX)-\bayespif(\mX)|
\lesssim
\left[R_{\ell,\pi}(f)-R_{\ell,\pi}(\bayesf)\right]^{\alpha}+
 }{\DIFadd{1\over \rho(\pi, \tN)}}\DIFadd{\left[R_{\ell,\pi}(f)-R_{\ell,\pi}(\bayesf)\right].
}\end{equation}
\DIFaddend \end{proof}
...
\mathbb{P}[g(\mX)\leq t] \leq \DIFdelbegin \DIFdel{C(Ht)}\DIFdelend \DIFaddbegin \DIFadd{Ct}\DIFaddend ^{\alpha/1-\alpha}, \quad \text{for all }t\in(0,\DIFdelbegin \DIFdel{\delta}\DIFdelend \DIFaddbegin \DIFadd{\rho}\DIFaddend ],
\end{align} 
for some constants $\alpha\in(0,1]$ and \DIFdelbegin \DIFdel{$t_*\in(0,1], C>0$}\DIFdelend \DIFaddbegin \DIFadd{$ C>0$}\DIFaddend . (When $\alpha=1$, the right hand side of~\eqref{eq:prob2} is interpreted as being zero.) Then, for all nonnegative, bounded functions $f\colon \tX\to\mathbb{R}_{\geq 0}$ with $\mnormSize{}{f}\leq L$,  the \DIFdelbegin \DIFdel{expected $f(\mX)$ is upper bounded by the $\alpha$-th power of the expected $f(\mX)g(\mX)$; i.e.,
}\DIFdelend \DIFaddbegin \DIFadd{following holds true,
}\DIFaddend \[
\DIFdelbegin \DIFdel{\mathbb{E}f(}%DIFDELCMD < \mX%%%
\DIFdel{) \leq CH^{\alpha} }%DIFDELCMD < [%%%
\DIFdel{\mathbb{E}f(}%DIFDELCMD < \mX%%%
\DIFdel{)g(}%DIFDELCMD < \mX%%%
\DIFdel{)}%DIFDELCMD < ]%%%
\DIFdel{^\alpha,
}\DIFdelend \DIFaddbegin \DIFadd{\mathbb{E}[f(\mX)g(\mX)]
}\gtrsim \begin{cases}
...
\begin{proof} Let $t>0$ be an arbitrary number in the interval \DIFdelbegin \DIFdel{$(0,t_*]$}\DIFdelend \DIFaddbegin \DIFadd{$(0,\rho]$}\DIFaddend . We bound the expectation of the product $fg$ using the integral over the region \DIFdelbegin \DIFdel{$\{\mX\colon g(\mX)> t/H\}$, 
}\DIFdelend \DIFaddbegin \DIFadd{$\{\mX\colon g(\mX)> t\}$, 
}\DIFaddend \begin{align}\label{eq:lowerbound}
\mathbb{E}[f(\mX)g(\mX)]& \geq  \DIFdelbegin \DIFdel{\mathbb{E}[f(\mX)\mathds{1}(g(\mX)>t/H)]}\DIFdelend \DIFaddbegin \DIFadd{\mathbb{E}[f(\mX)\mathds{1}(g(\mX)>t)]}\DIFaddend \notag \\
& = \DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend t\DIFdelbegin %DIFDELCMD < \over %%%
\DIFdel{H}%DIFDELCMD < }%%%
\DIFdel{\mathbb{E}[f(\mX)-f(\mX)\mathds{1}(g(\mX)\leq t/H)]
}\DIFdelend \DIFaddbegin \DIFadd{\mathbb{E}[f(\mX)-f(\mX)\mathds{1}(g(\mX)\leq t)]
}\DIFaddend \\
&\geq \DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend t\DIFdelbegin %DIFDELCMD < \over %%%
\DIFdel{H}%DIFDELCMD < }%%%
\DIFdelend \left(\mathbb{E}
f(\mX)-L\DIFdelbegin \DIFdel{\mathbb{P}[g(\mX)\leq t/H]}\DIFdelend \DIFaddbegin \DIFadd{\mathbb{P}[g(\mX)\leq t]}\DIFaddend \right)\notag \\
&\geq \DIFdelbegin %DIFDELCMD < {%%%
\DIFdel{1}%DIFDELCMD < \over %%%
\DIFdel{H}%DIFDELCMD < }\left(%%%
\DIFdelend t\mathbb{E}f(\mX)-CLt^{1/1-\alpha}\DIFdelbegin %DIFDELCMD < \right)%%%
\DIFdelend , \quad \text{for all }t\in(0,\DIFdelbegin \DIFdel{t_*}\DIFdelend \DIFaddbegin \DIFadd{\rho}\DIFaddend ],
\end{align}
%\fixme{Miaoyan}{the following proof needs to be updated to reflex the additional $H$..}
where the third line uses the fact that $f(\mX)\in[0,L]$, and the last line follows from~\eqref{eq:prob2}. We maximize the lower bound~\eqref{eq:lowerbound} with respect to \DIFdelbegin \DIFdel{$t\in(0,t_*]$ }\DIFdelend \DIFaddbegin \DIFadd{$t\in(0,\rho]$ }\DIFaddend and obtain the optimal \DIFdelbegin \DIFdel{$t_{\text{opt}}\in(0,t_*]$,
}\DIFdelend \DIFaddbegin \DIFadd{$t_{\text{opt}}\in(0,\rho]$,
}\DIFaddend \[
t_{\text{opt}}=\DIFdelbegin %DIFDELCMD < \begin{cases}
%DIFDELCMD < t_*, & \text{if}\quad \mathbb{E}f(\mX)\geq {CL\over 1-\alpha} t_*^{\alpha/ 1-\alpha},\\
%DIFDELCMD < \left[{1-\alpha \over CL}\mathbb{E}f(\mX)\right]^{(1-\alpha)/ \alpha}, & \text{if}\quad \mathbb{E}f(\mX) < {CL\over 1-\alpha} t_*^{\alpha
%DIFDELCMD < / 1-\alpha}.
%DIFDELCMD < \end{cases}%%%
\DIFdelend \DIFaddbegin \begin{cases}
...
\[
\mathbb{E}[f(\mX)g(\mX)]
\geq \DIFdelbegin %DIFDELCMD < \begin{cases}
%DIFDELCMD < \frac{\alpha t_*}{H} \mathbb{E}f(\mX), &\text{if}\quad \mathbb{E}f(\mX)\geq {CL \over 1-\alpha} t_*^{\alpha/ 1-\alpha},\\
%DIFDELCMD < \\
%DIFDELCMD < \frac{\alpha}{H}{\left(1-\alpha\over CL\right)}^{(1-\alpha) / \alpha} [\mathbb{E}f(\mX)]^{1/\alpha}, &\text{if}\quad \mathbb{E}f(\mX)< {CL\over 1-\alpha} t_*^{\alpha /1-\alpha}.
%DIFDELCMD < \end{cases}%%%
\DIFdelend \DIFaddbegin \begin{cases}
...
\end{cases}\DIFaddend 
\]
\DIFdelbegin \DIFdel{Hence, we conclude
}\[
\DIFdel{\mathbb{E}[f(\mX)g(\mX)]\geq \frac{C_2}{H}\min\left\{ \mathbb{E}f(\mX),\ [\mathbb{E}f(\mX)]^{1/\alpha}\right\},
}\]%DIFAUXCMD
\DIFdel{where $C_2>0$ is a constantindependent of $f$. When $\mathbb{E}f(\mX)\leq 1$, the right hand side of the above inequality takes value $C_2[\mathbb{E}(f(\mX)]^{1/\alpha}/H$. When $\mathbb{E}f(\mX)>1$, we rescale $f$ by $f/L$ and obtain
}\[
\DIFdel{\mathbb{E}[f(\mX)g(\mX)]\geq \frac{C_2}{H}L^{-(\alpha-1)/\alpha}[\mathbb{E}f(\mX)]^{1/\alpha}.
}\]%DIFAUXCMD
\DIFdel{Combining both cases gives }\DIFdelend \DIFaddbegin \DIFadd{Since, $\alpha, C$, and $L$ are given constant, we have }\DIFaddend the desired results.
\DIFaddbegin 

%DIF > \[
%DIF > \mathbb{E}[f(\mX)g(\mX)]\geq \frac{C_2}{H}\min\left\{ \mathbb{E}f(\mX),\ [\mathbb{E}f(\mX)]^{1/\alpha}\right\},
%DIF > \]
%DIF > where $C_2>0$ is a constant independent of $f$. When $\mathbb{E}f(\mX)\leq 1$, the right hand side of the above inequality takes value $C_2[\mathbb{E}(f(\mX)]^{1/\alpha}/H$. When $\mathbb{E}f(\mX)>1$, we rescale $f$ by $f/L$ and obtain
%DIF > \[
%DIF > \mathbb{E}[f(\mX)g(\mX)]\geq \frac{C_2}{H}L^{-(\alpha-1)/\alpha}[\mathbb{E}f(\mX)]^{1/\alpha}.
%DIF > \]
%DIF > Combining both cases gives the desired results. 
\DIFaddend \end{proof}
