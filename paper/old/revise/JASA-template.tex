...
\label{sec:intro}
%Our research is motivated by some important applications, where the matrix-valued covariateshave an important attribute that the variables in the same row (column) share similar information or are associated with a common (latent) factor. For example, in electroencephalography (EEG) data, variables in the same row are associated with the same location, and variables in the same column are associated with the same frequency of electrode. Selection of the location and frequency is of interest in application. Also in Excitation–emission matrix (EEM) data, the variables of the same row and column are associated with the same excitation and emission wavelength, respectively. And selection of these wavelength is important in applications. Another example is the analysis of colorimetric sensor array data (Zhong and Suslick, 2015), where the rows of the data represent the dye affect and columns stand for the spectrum of color. One would like to select the important dyes and spectrums. In these applications, one would like to select the variables in rows and columns.
...
%curse of dimensionality in non-parametric regression
%Solution: semiparametric model: partially linear model）a+b1X1+m2(X2)[6], additive mode a+m1(X1)+...+md(Xd)[7], single index model）m(b1X1+...+bdXd), varying coefficient model）b1(T1)X1+...+bd(Td)Xd[9]
...
%8. Härdle, Wolfgang, Hall, Peter, and Ichimura, Hidehiko (1993) Optmal smoothing in single-index models. Ann. Statist. 21, 151–178
%9. Hastie, Trevor and Tibshirani, Rob (1993) Varying-coefficient models. J. Roy. Stat.Soc. Ser. B. 55, 757–796
...
%Let 
%\[
%p(\mx)=\mathbb{E}(Y|\mX=\mx),\quad \mx\in\tX.
%\]
%denote the regression function. 
...
%Take a countable sequence of $\tJ$ covers the interval $[-1,1]$. A natural estimation of $\eta(\mX)$ is
%\[
