...
\usepackage{authblk}
\title{Appendix for ``Nonparametric Trace Regression in High Dimensions via Sign Series Representation''}
\author[1]{Chanwoo Lee}
\author[2]{Lexin Li}
\author[3]{Hao Helen Zhang}
\author[1]{Miaoyan Wang}
\affil[1]{University of Wisconsin-Madison}
\affil[2]{University of California-Berkley}
...
\end{equation}
Let $(\hat \mB, \hat b)$ be an arbitrary global optimizer of~\eqref{eq:phi}. Write $\hat \phi(\mX_i)=\langle \mX_i, \hat \mB \rangle+\hat b$, and $\bar Y_i=\bar Y_{\pi,i}$ for all $i\in[n]$. If $(\hat \mB, \hat b)$ satisfies~\eqref{eq:b}, then we keep this $(\hat \mB, \hat b)$. Otherwise, we aim to construct another global optimizer that satisfies~\eqref{eq:b}. The construction is divided into two cases based on loss functions. 
\begin{enumerate}[label={2.\arabic*},wide, labelwidth=!, labelindent=0pt]
\item[Case 1:] $F$ is 0-1 loss or psi-loss. 

Denote
\[
i^*=\argmin_{i\in[n]}|\hat \phi(\mX_i)|, \quad \text{and}\quad r:= \min_{i\in[n]}|\hat \phi(\mX_i)|=|\hat \phi(\mX_{i^*})|>1.
\]
We construct a shifted trace function,
\[
\phi^*\colon \mX\mapsto \hat \phi(\mX) - (r-1)\sign\hat \phi(\mX_{i^*}) = \KeepStyleUnderBrace{\langle \mX, \hat \mB \rangle}_{\text{trace}}+\KeepStyleUnderBrace{\hat b -(r-1)\sign\hat \phi(\mX_{i^*}) }_{\text{new intercept $=:\hat b^*$}}.
\]
The assumption $r>1$ implies that, for each $i\in[n]$, $\hat \phi(\mX_i) \sign \bar Y_{\pi, i}$ is either $\geq r>1$ or $\leq -r <-1$. By the definition of $\phi^*$ and loss function $F$, we have
\[
F\left(\phi^*(\mX_i)\sign \bar Y_{i}\right)=
\begin{cases}
F\big(\hat \phi(\mX_i)\sign \bar Y_{i}\big)=0,   & \text{if $\hat \phi(\mX_i) \sign \bar Y_{i}\geq r>1$},\\
F\big(\hat \phi(\mX_i)\sign \bar Y_{i}\big)=1,   & \text{if $\hat \phi(\mX_i) \sign \bar Y_{i}\leq r< -1$}.\\
\end{cases}
\]
...
\end{cases}
\quad \quad \text{and}\quad \quad r:= |\hat \phi(\mX_{i^*})|>1,
\]
Notice that the construction of $i^*$ ensures $(L_+-L_-)\sign \hat \phi(\mX_{i^*})=-|L_+-L_-|$. We construct a shifted trace function
\begin{equation}\label{eq:new}
\phi^*\colon \mX\mapsto \hat \phi(\mX) - (r-1)\sign\phi(\mX_{i^*}) = \KeepStyleUnderBrace{\langle \mX, \hat \mB \rangle}_{\text{trace}}+\KeepStyleUnderBrace{\hat b -(r-1)\sign\hat \phi(\mX_{i^*}) }_{\text{intercept}}.
\end{equation}
By construction, 
\[
F\left(\phi^*(\mX_i)\sign \bar Y_{i}\right)=
\begin{cases}
F\left(\hat \phi(\mX_i)\sign \bar Y_{i}\right)=0,   & \text{if $\hat \phi(\mX_i) \sign \bar Y_i\geq r>1$},\\
F\left(\hat \phi(\mX_i)\sign \bar Y_{i}\right)+(r-1)\sign\hat \phi(\mX_{i^*}),   & \text{if }i\in \tI_{+},\\
F\left(\hat \phi(\mX_i)\sign \bar Y_{i}\right)-(r-1)\sign\hat \phi(\mX_{i^*}),   & \text{if }i\in \tI_{-}.\\
\end{cases}
\]
Therefore $\phi^*$ defined in~\eqref{eq:new} is a global optimizer of~\eqref{eq:phi}, since
\[
\sum_{i\in[n]}|Y_i|F(\hat \phi^*(\mX_i)\sign \bar Y_i) =\sum_{i\in[n]}|Y_i|F(\hat \phi(\mX_i)\sign \bar Y_i) - (r-1)|L_{+}-L_{-}| \leq \sum_{i\in [n]}|Y_i|F(\hat \phi(\mX_i)\sign \bar Y_i).
\]
...
Assume, for simplicity, $\mathbb{P}\left(\FnormSize{}{\mX}\leq 1\right)=1$. For any given $k\geq 1$, let $\Phi^k=\{f\in \Phi(r,s_1,s_2)\colon \FnormSize{}{\phi} \leq k\}$ denote the sub-class of functions with coefficient magnitudes bounded by $k$. Then, 
\[
\tH_{[\ ]}(\varepsilon,\ \Phi^k,\ \vnormSize{}{\cdot}) \lesssim r(s_1+s_2) \log {kd_{\max}\over \varepsilon }.
\]
Furthermore, when we consider $\Phi^k = \{\phi\in\Phi(r)\colon \FnormSize{}{\phi}\leq k \}$ with $(s_1,s_2) = (d_1,d_2)$, then
\[
...
0 & b\\
 \end{bmatrix}
 \in\mathbb{R}^{(d_1+1) \times (d_2+1)} \colon \rank(\mB)\leq r,\ \supp(\mB)\leq (s_1,s_2),\ |b|\leq k+1, \ \FnormSize{}{\mB}\leq  k
 \right\}
 \end{align}
...
\]
where $\tH$ denotes the log covering number for the (non-bracketing) set. Therefore, it suffices to bound $\tH(\varepsilon,\ \tB,\ \FnormSize{}{\cdot})$ where $\tB$ is included in a $(\sqrt{3}k)$-ball by definition of $\tB$. Now fix two subsets $S_1,S_2\subset [d]$ with $|S_1|=s_1$ and $|S_2|=s_2$, where $|\cdot|$ denotes the cardinality of the sets. Let $\tB_{S_1,S_2}\subset \tB$ denote the subset of matrices satisfying $\mB(i,j)=0$ whenever $(i,j)\notin S_1\times S_2$. Based on~\citet[Lemma 3.1]{candes2011tight}, the log covering number for $\tB_{S_1,S_2}$ is
\begin{align}\label{eq:bracketsparse}
\tH \left(\varepsilon,\ \tB_{S_1,S_2},\ \Fnorm{\cdot}\right)\lesssim r(s_1+s_2+1)\log\left({\sqrt{k}\over \varepsilon}\right).
\end{align}
...
Define $\Phi^{k}=\{f\in\Phi(r,s_1,s_2)\colon \FnormSize{}{f} \leq k\}$ for all $k\geq 1$; i.e., $\Phi^k$ is the subset of functions in $\Phi(r,s_1,s_2)$ with coefficient magnitudes bounded by $k$. Set 
