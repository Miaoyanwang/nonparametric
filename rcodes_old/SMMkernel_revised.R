...
polykernel = function(Y,Z,deg = 3){
n = ncol(Y)
return(matrix.power(t(Y)%*%Z+diag(1,n),deg))
}

m=10
n=2
result = gendat(m,n,2,20,b0)
X = result$X
K = Karray(X,polykernel)

x=t(solve(sqrt_matrix(K[1,1,,])))%*%K[1,2,,]%*%solve(sqrt_matrix(K[2,2,,]))
t(x)%*%x

K[2,1,,]%*%ginv(K[1,1,,])%*%K[1,2,,]==K[2,2,,]

sqrt_matrix=function(M){
    res=eigen(M)
    return(t(res$vector%*%diag(sqrt(pmax(res$value,10^-4)))))
}
...
#test2=cbind(diag(2),sqrt(2)*t(X[[2]]),t(X[[2]])%*%X[[2]])
#test=cbind(diag(2),sqrt(2)*t(X[[1]]),t(X[[1]])%*%X[[1]])

#M2=M
#M2[1,2]=tr(test%*%t(test2))
#M2[2,1]=tr(test2%*%t(test))

#delta=t(X[[1]])%*%X[[2]]-t(X[[2]])%*%X[[1]]
#tr(X[[1]]%*%delta%*%t(X[[2]]))
