...
indset = matrix(0,nrow =204 ,ncol = 2)
s = 0
for(i in 1:68){
  for(j in 1:3){
    s = s+1
    indset[s,]= c(i,c(0.01,0.1,1)[j])
  }
}


cvresult = matrix(nrow = 5, ncol = 6)
colnames(cvresult) = c("trainzloss","trainhloss","zloss","hloss","rank","cost")


r = indset[BATCH,][1]
cost = indset[BATCH,][2]
cvresult[1:5,5:6] = matrix(rep(c(r,cost),5),nrow = 5,byrow = T)

for(k in 1:5){
  test_index = cindex[[k]]
  train_index = setdiff(1:114,test_index)
  train_X = X[train_index]; train_y =  y[train_index]
  con=SMMK_con(train_X,train_y,r,kernel_row="linear",kernel_col="linear",cost, rep = 10, p = .5)
  
  #0-1 loss
  predict = unlist(lapply(X,con$predict))
  trainy_predict = predict[train_index]
  testy_predict = predict[test_index]
  cvresult[k,1] = length(which(trainy_predict-y[train_index]!=0))/length(trainy_predict)
  cvresult[k,3] = length(which(testy_predict-y[test_index]!=0))/length(testy_predict)
  
  #hinge loss
  hloss = hinge(unlist(lapply(X,function(x) con$slope(x)+con$intercept))*y)
  cvresult[k,2] = sum(hloss[train_index]); cvresult[k,4] = sum(hloss[test_index])
...
save(cvresult,file = paste("CVIQ_",r,"_",cost,".RData",sep = ""))
