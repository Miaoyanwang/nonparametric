...
\end{align}
The following theorem states the accuracy for sign function estimate $\sign(\hat \phi_{\pi,F})\colon \tX\to\{-1,1\}$. We say that an event $A$ occurs ``with high probability'' if $\mathbb{P}(A)$ tends to 1 as the dimension $d\to \infty$.
...
\item[(c)] (Theorem~\ref{thm:extension}) Surrogate loss satisfying Assumption~\ref{ass:main}, constant $(s_1,s_2)$, $t_n={1\over n} r(s_1+s_2) \log d_{\max}$, penalization $\lambda\asymp (t^{(\alpha+1)/(\alpha+2)}_n+t_n /\rho )$, approximation error $a^{(\alpha+1)/(\alpha+2)}_n \leq t_n$.
\end{enumerate}
...
\]
where $\text{Risk}_{F'}(\phi):=\mathbb{E}\left[|\shift|F'( \phi(\mX)\sign\shift)\right]$ denotes the risk evaluated under $T$-truncated hinge loss $F'=\min(T,\ (1-z)_{+})$, and $T=\max(2,J) \geq \max(2, \|{\phi_\pi^{(n)}}\|_{\infty})$ is a constant based on Assumption~\ref{ass:main}(a). 
\end{enumerate}
\end{lem}
\begin{proof}[Proof of Lemma~\ref{lem:risk}]
 In Theorem~\ref{thm:main} and~\ref{thm:sparse}, we use 0-1 loss and have  $\lambda = a_n = 0$.  First, we consider the more general case in Theorem~\ref{thm:extension}, then come back to the setting in  Theorem~\ref{thm:main} and~\ref{thm:sparse}. 
 

Case 1: When $\lambda\asymp (t_n^{(\alpha+1)/(\alpha+2)}+t_n/\rho),$ and $ a_n\leq t_n$ (Theorem~\ref{thm:extension}) .
...
&\sup_{\phi\in A_{s,k}}\textup{Var}\left[\ell_{\pi,F}(\phi_\pi^{(n)};(\mX,Y))-\ell_{\pi,F}(\phi;(\mX,Y)\right]\\&\leq\sup_{\phi\in A_{s,k}}2\left\{ \textup{Var}\left[\ell_{\pi,F}(\phi_\pi^{(n)};(\mX,Y))-\ell_{\pi,F}(\bayespif;(\mX,Y)\right]+\textup{Var}\left[\ell_{\pi,F}(\phi;(\mX,Y))-\ell_{\pi,F}(\bayespif;(\mX,Y)\right]\right\}\notag \\
...
\end{cases}
\]
Notice that we have the condition  $\lambda\asymp  t_n^{(\alpha+1)/(\alpha+2)}+{1\over \rho} t_n$ in  Theorem~\ref{thm:unified}. From this $\lambda$,  we choose $L_n$ with suitable constant factor such that $\lambda J\leq L_n/2$. Remember this condition guarantees $M(s,k)>0$ for all $s,k\in\mathbb{N}.$
...
,
\end{align}
where the third line comes from  $n\rho L_n\asymp n\rho\lambda J  \gg0$ by \eqref{eq:delta} and  the last inequality uses the fact that $\rho L_n\geq t_n$. The proof is then complete by bounding the 0-1 risk by $F$-risk. 
...
\subset &
 ...
 \widehat{\textup{Risk}}_{\pi,F'}(\phi)+\lambda\FnormSize{}{\phi}^2\leq  \widehat{\textup{Risk}}_{\pi,F'}(\phi_{\pi}^{(n)})+\lambda J \bigg\}
.
\end{align}
...
\]
because the truncation constant is $T= \max(2,J)>\max(2,\sup_n\|{\phi_\pi^{(n)}}\|_{\infty})$. Notice that the last line is exactly the same with \eqref{eq:outer} except $F$ being replaced by $F'$. 
The remaining proof follows the same line of argument as the bounded case.  In particular, we invoke the mean-to-variance relationship for bounded $F'$-loss in~\eqref{eq:second}. The final conclusion follows from the excess bound inequality for $T$-truncated risk (c.f. Lemma~\ref{lem:prepare}).


Case 2: When $\lambda = 0$ and $a_n = 0$ with 0-1 loss (Theorem~\ref{thm:main} and~\ref{thm:sparse}).
...
\Gamma :=&\left\{(\mX_i,Y_i)_{i\in[n]}\colon \risk(\hat\phi_{\pi})-\risk(\bayespif)\geq L_n \right\}\notag \\
 \subset &
 \left\{(\mX_i,Y_i)_{i\in[n]}\colon \exists \phi\in\Phi(r,s_1,s_2), \text{s.t.}\  \risk(\phi; (\mX,Y))-\risk(\bayespif)\geq L_n\text{ and}\  \erisk(\phi)\leq \erisk(\bayespif)\right\}\notag \\
 \subset &
\left\{(\mX_i,Y_i)_{i\in[n]} \colon \sup_{\substack{\phi\in\Phi(r,s_1,s_2)\\  
...
Applying Lemma~\ref{lem:metric} with  finite $k = 1$ and $\lambda = 0$  shows that $L_n\asymp t_n^{(\alpha+1)/(\alpha+2)}+ t_n/\rho$ satisfies the conditions in \citet[Theorem 3]{shen1994convergence}, where 
\[
t_n=\begin{cases}
{rd_{\max}\over n}, \quad \text{low-rank model $\phi\in\Phi(r)$,}\\
{r(s_1+s_2)\log d_{\max}\over n}, \quad \text{low-rank and two-way sparse model $\phi\in\Phi(r,s_1,s_2)$.}\\
\end{cases}
\]
Therefore,   it follows from \citet[Theorem 3]{shen1994convergence} and \eqref{eq:01bd} that 
\begin{align}
\Gamma&\lesssim  \sum_{s} \exp\left(-nM^2(s)\over V(s)+M(s)\right)\\&\lesssim \sum_{s}\exp(-\rho snL_n)\\&\leq \left(e^{-n\rho L_n}\over 1-e^{-n\rho L_n}\right)\\&\lesssim e^{-n\rho L_n}\\&\leq e^{-nt_n},
\end{align}
where the fourth line comes from  $n\rho L_n\gg0$ by our choice of $L_n$ and  the last inequality uses the fact that $\rho L_n\geq t_n$. 
\end{proof}
...
\end{equation}
with high probability at least $1-\exp(-nt)$ for any $t\geq t_n$.
\end{rmk}
...
\end{align}
where the first inequality uses the property of $A$, and the last inequality follows from Lemma~\ref{lem:H}.  Combining the bounds for the last two terms in \eqref{eq:twobounds} and plugging it into \eqref{eq:pfmain} yields, 
\begin{equation}
\mathbb{P}\left(\onenormSize{}{\hat f-f}\lesssim t^{\alpha/(\alpha+2)}+{1\over H}+tH\right)\geq\mathbb{P}(A),
\end{equation}
for any $t\geq t_n$.
...
\right)\\&\gtrsim 1-(2H+1)\exp(-nt)\gtrsim 1-\exp(-nt+\log H).
\end{align*}
Choosing $t \asymp t_n\log H$ so that $\log H$ is negligible compared to $nt$, we obtain 
\begin{align*}
\onenormSize{}{\hat f-f}\lesssim (t_n\log H)^{\alpha/(\alpha+2)}+{1\over H}+t_nH\log H,
\end{align*}
with probability at least $1-\exp(-nt_n\log H)\geq 1-\exp(-nt_n)$.  Setting $H\asymp t^{-1/2}_n$ yields the desired upper bound. 
...
\[
\tH_{[\ ]}(\varepsilon,\ \Phi^k,\ \vnormSize{}{\cdot}) \leq  Cr(s_1+s_2) \log {kd_{\max}\over \varepsilon }.
\]
Furthermore, when we consider $\Phi^k = \{f\in\Phi(r)\colon \FnormSize{}{f}^2\leq k \}$, where $(s_1,s_2) = (d_1,d_2)$,  there exists a constant $C>0$ such that
\[
\tH_{[\ ]}(\varepsilon,\ \Phi^k,\ \vnormSize{}{\cdot}) \leq  Crd_{\max} \log {k\over \varepsilon }.
\]
\end{lem}
\begin{proof} 
...
The result in $(s_1,s_2) = (d_1,d_2)$ case is directly followed from \eqref{eq:bracketsparse}.
...
\end{lem}
\begin{rmk}
The local complexity result for $\Phi(r)$ is the same  except $\log d_{\max}$  being removed in $L_n$.
\end{rmk}
\begin{proof}
