...
$\bayesS(\pi)=\{\mX\in\tX\colon f(\mX)\leq \pi\}$ & Indicator set corresponding to $\bayespif$. \\
$r$& matrix rank\\
...
$\phi$ & an arbitrary classifier function from $\tX$ to $\{-1,1\}$\\
...
$\ell_{\pi,F}$ & weighted $F$-loss function from $\mathbb{R}$ to $\mathbb{R}_{\geq 0}$\\
...
d_\pi(S,\bayesS)\geq t\left( \mathbb{P}_{\mX}\left(S\Delta S_{\text{bayes}}\right) - Ct^{\alpha\over 1-\alpha}\right),\quad \text{for all }0\leq t < \rho(\pi,\tN).
\end{equation}
We maximize the lower bound of~\eqref{eq:tail2} with respect to $t$, and obtain the optimal $t_{\text{opt}}$,
\[
t_{\text{opt}}=\begin{cases}
\rho(\pi,\tN), & \text{if}\quad \mathbb{P}_{\mX}\left(S\Delta S_{\text{bayes}}\right)> 2C(1+\alpha)\rho^\alpha(\pi,\tN),\\
\left[{1\over 2C(1+\alpha)}\mathbb{P}_{\mX}\left(S\Delta S_{\text{bayes}}\right)\right]^{1 / \alpha}, & \text{if}\quad \mathbb{P}_{\mX}\left(S\Delta S_{\text{bayes}}\right)\leq 2C(1+\alpha) \rho^\alpha(\pi,\tN).
\end{cases}
\]
The corresponding lower bound of the inequality~\eqref{eq:tail2} becomes
\begin{align}
  d_\pi(S,\bayesS)\geq\begin{cases}
c_1\rho(\pi,\tN)\mathbb{P}_{\mX}(S\Delta \bayesS), & \text{if}\quad \mathbb{P}_{\mX}\left(S\Delta S_{\text{bayes}}\right)> 2C(1+\alpha)\rho^\alpha(\pi,\tN),\\
c_2 \left[\mathbb{P}_{\mX}\left(S\Delta S_{\text{bayes}}\right)\right]^{1+\alpha \over \alpha}, & \text{if}\quad \mathbb{P}_{\mX}\left(S\Delta S_{\text{bayes}}\right)\leq 2C(1+\alpha) \rho^\alpha(\pi,\tN),
\end{cases}
\end{align}
...
\[
\onenormSize{}{\phi-\bayespif} \leq \left[\riskF(\phi)-\riskF(\bayespif)\right]^{\alpha \over 1+\alpha}+{1\over \rho(\pi, \tN)}\left[\riskF(\phi)-\riskF(\bayespif)\right]
\]
\item For $T$-truncated hinge loss $F(z)=\min((1-z)_{+},T)$ with $T\geq 2$,
\[
\onenormSize{}{\phi^T-\bayespif} \leq \left[\riskF(\phi)-\riskF(\bayespif)\right]^{\alpha \over 1+\alpha}+{1\over \rho(\pi, \tN)}\left[\riskF(\phi)-\riskF(\bayespif)\right],
\]
...
\lesssim &\ 
\left[\riskF(\phi)-\riskF(\bayespif)\right]^{\alpha/(1+\alpha)}+ {\over \rho(\pi, \tN)}\left[\riskF(\phi)-\riskF(\bayespif)\right].
 \end{align}
 \end{itemize}
\end{lem}
\begin{rmk}
...
\end{align}
where the second line comes from the boundedness of 0-1 loss, and the third line comes from the boundedness of weight $|\shift|$, and fourth line comes from the inequality $||a-b|-|c-b||\leq |a-b|$ for $a,b,c\in\{-1,1\}$. Here we have absorbed the constant multipliers in $\lesssim$. Therefore, the conclusion~\eqref{eq:b1} then directly follows by applying Remark~\ref{eq:rmk} to~\eqref{eq:mae}. 

\item[Case 2:] $F(z)=(1-z)_{+}$ is hinge loss. 

Property (a) was firstly introduced in~\citet[Lemma 1]{wang2008probability}, and here we provide an alternative proof. A direct calculation (c.f.\ Lemma~\ref{lem:hingeL1}) shows that
\[
...
\]
To put our problem in the context of~\cite{scott2011surrogate}, we need addition notation. For any function measurable $g\colon x\mapsto g(x)$, we write $g=g^{+}-g^{-1}$, where $g^{+}$ and $g^{-}$ are two non-negative functions given by
\begin{align}
g^{+}(x)=\max\{ g(x),0 \} =
\begin{cases}
g(x), & \text{if }g(x)>0,\\
...
\begin{align}
\text{(i)}=\mathbb{E}\left|\sign (\phi(\mX)\shift)-\sign (\bayespif(\mX)\shift)\right|&\lesssim d_\Delta(\bayesS(\pi),S)\\&\lesssim  d^\alpha_{\pi}(\bayesS(\pi),S)+{1\over \rho(\pi, \tN)}d_{\pi}(\bayesS(\pi),S),
\end{align}
where the first line uses the fact that $F(1)=0$ and $F(-1)=2$, and last inequality is from Theorem~\ref{thm:identifiability}.
...
\begin{align}
    ...
    & \quad + \mathbb{E}\left[|\shift|(1-\sign(\phi(\mX)\shift))-|\shift|(1-\sign(\bayespif(\mX)\shift))\right]\\
    &\leq [\riskF(\phi)-\riskF(\bayespif)]+d_\pi(\bayesS(\pi),S),
\end{align}
where the first inequality is based on $F(z)\geq 1-\sign(z)$, and the last inequality is from definition of $d_\pi(\cdot,\cdot)$. The proof is complete by combining \eqref{eq:excess-hinge}, \eqref{eq:b1} and bounds (i)-(ii).
...
\begin{align}
&\text{Var}\left\{|\shift|\left[F(\sign \shift\phi(\mX))-F(\sign \shift\bayespif(\mX)\right]\right\}\\
\leq&\ 4 \mathbb{E}|F(\sign \shift\phi(\mX))-F(\sign \shift\bayespif(\mX))|^2\\
\lesssim &\ T\mathbb{E}|F(\sign \shift\phi(\mX))-F(\sign \shift\bayespif(\mX))|\\
\lesssim &\ T\mathbb{E}|\phi^T-\bayespif|
\end{align}
where $T>0$ is the upper bound of truncated hinge loss , the second line comes from the 1-Lipschitz continuity of the $T$-truncated hinge loss, and the last line comes from the definition of $F$. Applying Remark~\ref{rmk:truncate} in Lemma~\ref{lem:hingeL1} on the last inequality complete the proof.
...
The proof adopts the local iterative techniques from~\citet[Theorem 3]{wang2008probability} (also see Section~\ref{sec:auxiliary}). Similar techniques have been used in \citet[Theorem 4]{bartlett2006convexity} for similar estimate but without ridge penalization.
...
\]
where $\text{Risk}_{F'}(\phi):=\mathbb{E}\left[|\shift|F'(\sign\shift \phi(\mX))\right]$ denotes the risk evaluated under $T$-truncated hinge loss $F'=\min(T,\ (1-z)_{+})$, and $T=\max(2,J) \geq \max(2, \sup_n\mnormSize{}{f_\pi^{(n)}})$ is a constant based on Assumption~\ref{ass:main}(a). 
\end{enumerate}
\end{lem}
...
Fix $\pi\notin\tN$, and write $\rho=\rho(\pi,\tN)$. For any function $\phi\in\Phi(r,s_1,s_2)$ of consideration, define the excess $F$-loss
\begin{equation}\label{eq:F}
\Delta(\phi;(\mX,Y)) = \ell_{\pi,F}(\phi; (\mX,Y))-\ell_{\pi,F}(\bayespif; (\mX,Y)),
\end{equation}
Correspondingly, define the empirical excess $F$-risk
\begin{equation}\label{eq:def1}
 \mathbb{\hat E}\Delta(\phi;(\mX,Y))={1\over n}\sum_{i\in[n]}\Delta(\phi;(\mX_i,Y_i)).
\end{equation}
Under the notation, our estimate $\hat \phi_{\pi,F}$ is the minimizer of the regularized empirical excess $F$-risk,
\begin{equation}\label{eq:def2}
\hat \phi_{\pi,F}=\argmin_{\phi\in\Phi(r,s_1,s_2)}\Big\{ \mathbb{\hat E}\Delta(\phi;(\mX,Y)) +\lambda\FnormSize{}{\phi}^2\Big\}.
\end{equation}
We are interested in the convergence of $\hat \phi_{\pi,F}$'s  excess risk, $\mathbb{E}\Delta(\hat\phi_{\pi,F};(\mX_{\text{new}},Y_{\text{new}}))$, where the expectation is taken with respect a refreshed test point $(\mX_{\text{new}},Y_{\text{new}})\sim \mathbb{P}_{\mX,Y}$. 

Let $L_n$ denote the convergence rate to seek. By the definition of $\hat \phi_{\pi,F}$ and Assumption~\ref{ass:main}(a), we have
\[
 \mathbb{\hat E}\Delta (\hat \phi_{\pi,F};(\mX,Y)) +\lambda|\hat \phi_{\pi,F}|\leq \lambda J+a_n,
\]
where $a_n\to 0$ is the approximation error. Therefore, we have the following inclusion of probability events,
\begin{align}\label{eq:outer}
&\left\{(\mX_i,Y_i)_{i\in[n]}\colon \mathbb{E}\Delta (\hat \phi_{\pi,F}; (\mX,Y))\geq L_n \right\}\notag \\
 \subset &
 \left\{(\mX_i,Y_i)_{i\in[n]}\colon \exists \phi\in\Phi(r,s_1,s_2), \text{s.t.}\  \mathbb{E}\Delta (\phi; (\mX,Y))\geq L_n \ \text{and}\  \mathbb{\hat E}\Delta (\phi;(\mX,Y)) +\lambda|\phi|- \lambda J\leq a_n \right\}\notag \\
 \subset &
\left\{(\mX_i,Y_i)_{i\in[n]} \colon \sup_{\substack{\phi\in\Phi(r,s_1,s_2)\\  \mathbb{E}\Delta (\phi; (\mX,Y))\geq L_n  }}\left[-\mathbb{\hat E}\Delta (\phi; (\mX,Y))-\lambda|\phi|+\lambda J+a_n \right]\geq 0 \right\} \notag \\
\subset & \bigcup_{\phi\in A_{s,k}}\{(\mX_i,Y_i)_{i\in[n]}\colon \sup_{\phi\in A_{s,k}} \left[-\mathbb{\hat E}\Delta(\phi;(\mX,Y))-\lambda|\phi|+\lambda J+a_n \right]\geq 0 \},
\end{align}
where $\mathbb{E}(\cdot)$ in the first line is taken with respect to a refreshed test data point $(\mX_{\text{new}},Y_{\text{new}})$. In the last line of~\eqref{eq:outer}, we have partitioned the set $\{\phi\in\Phi(r,s_1,s_2)\colon \mathbb{E}\Delta(\phi;(\mX,Y))\geq L_n\}$ into a union of $A_{s,k}$, with 
\begin{align}
A_{s,k}&=\{\phi\in \Phi(r,s_1,s_2)\colon sL_n\leq  \mathbb{E}\Delta (\phi; (\mX,Y)) < (s+1) L_n, (k-1) J \leq |\phi|< k J\},
\end{align}
for $s,k=1,2,\ldots$. 


Let $\Gamma$ denote the target probability for the first line in~\eqref{eq:outer}. To bound $\Gamma$, it suffices to bound the sum of probabilities over sets $A_{s,k}$. Towards this end, we need to bound the first and second moments of $\Delta (\phi; (\mX,Y))$ over $\phi\in A_{s,k}$. 

For the first moment, using the definition of $A_{s,k}$, we have
\begin{align}\label{eq:first}
&\inf_{A_{s,k}} \mathbb{E}\left[\Delta (\phi; (\mX,Y))+\lambda|\phi|-\lambda |\phi^*_{\pi}|\right] \geq sL_n+\lambda (k-2)J=:M(s,k)>0,
\end{align}
for all $s,k\in\mathbb{N}$, where the inequality $M(s,k)>0$ follows from the condition $\lambda J \leq L_n/2$. 

For the second moment, it follows from Lemma~\ref{lem:prepare} that
\begin{align}\label{eq:second}
\sup_{A_{s,k}}\textup{Var}\left[\Delta (\phi; (\mX,Y))\right] &\lesssim  \sup_{A_{s,k}}\left\{ \left[\mathbb{E}\Delta (\phi; (\mX,Y))\right]^{\alpha/(1+\alpha)}+{1\over \rho}\mathbb{E}\Delta(\phi;(\mX,Y))\right\}\notag \\
&\lesssim [M(s,k)]^{\alpha/(1+\alpha)}+{M(s,k)\over \rho}=:V(s,k),
\end{align}
provided the loss function is bounded. By~\eqref{eq:outer} and~\eqref{eq:first}, we obtain
\begin{align}\label{eq:union}
\Gamma\leq \sum_{s,k=1}^\infty\mathbb{P}\left\{\sup_{A_{s,k}}  \KeepStyleUnderBrace{\left[\mathbb{E}\Delta (\phi; (\mX,Y))-\mathbb{\hat E}\Delta (\phi; (\mX,Y))\right]}_{\text{empirical process indexed by $\phi\in A_{s,k}$}}\geq M(s,k)\right\},
\end{align}
where we have used the assumption that $a_n\leq L_n\leq M(s,k)$. We next bound the right-hand-side of~\eqref{eq:union} using standard empirical process theory. Specifically, we choose an $L_n$ that satisfies the conditions in~\citet[Theorem 3]{shen1994convergence}; see Section~\ref{sec:auxiliary}. (The specification of $L_n$ is deferred to the next paragraph). Once such $L_n$ is chosen, then it follows from~\citet[Theorem 3]{shen1994convergence} that
...
&\leq {e^{-n\rho L_n}\over 1-e^{-n\rho L_n} } \left(e^{-n\rho \lambda J} + 1+{e^{-n\rho \lambda J} \over 1-e^{-n\rho \lambda J}}\right) \leq e^{-n \rho L_n}.
\end{align}
where the last inequality uses the fact $n^{-1}\lesssim  \lambda J$. Notice that this bound for $\Gamma$ has a sub-exponential decay in $L_n$. 
...
&=\mathbb{P}\left[ \mathbb{E}\Delta(\hat \phi_{\pi,F};(\mX_{\text{new}},Y_{\text{new}}))\geq L_n   \right] \\
&\lesssim \exp(-n\rho L_n) \lesssim \exp(-nt_n).
%& \lesssim
%\begin{cases} 
%\exp\left( - Crd_{\max} \right), & \text{ low-rank model $\phi\in\Phi(r)$,}\\
%\exp\left( - Cr(s_1+s_2)\log d \right)\lesssim  d^{-Cr(s_1+s_2)}, & \text{ low-rank and two-way sparse model $\phi\in\Phi(r,s_1,s_2)$,}\\
%\end{cases}
\end{align}
...
For unbounded loss (hinge loss), we seek to bound the $F'$-risk of $\hat \phi_{\pi,F}$. The general strategy is to evaluate $\phi_{\pi,F}$'s error using $F'$-risk, where $F'$ is $T$-truncated version of $F$. For ease of notation, we add the dependence of $F'$ to the definition of $\Delta=\Delta(F')$ in~\eqref{eq:L}. We default $\Delta=\Delta(F')$ throughout  the proof, but write $\bar \Delta=\Delta(F)$ only when needed. Note that the estimate $\hat \phi_{\pi,F}$~\eqref{eq:def2} is defined under $\bar \Delta$. Therefore, the inclusion~\eqref{eq:outer} changes to
\begin{align}
&\left\{(\mX_i,Y_i)_{i\in[n]}\colon \mathbb{E}\Delta (\hat \phi_{\pi,F}; (\mX,Y))\geq L_n \right\}\notag \\
 \subset 
&\left\{(\mX_i,Y_i)_{i\in[n]}\colon \exists \phi\in\Phi(r,s_1,s_2), \text{s.t.}\  \mathbb{E}\Delta (\phi; (\mX,Y))\geq L_n \ \text{and}\  \mathbb{\hat E}\bar \Delta (\phi;(\mX,Y)) +\lambda|\phi|- \lambda J\leq a_n \right\}\\
 \subset
 & \left\{(\mX_i,Y_i)_{i\in[n]}\colon \exists \phi\in\Phi(r,s_1,s_2), \text{s.t.}\  \mathbb{E}\Delta (\phi; (\mX,Y))\geq L_n \ \text{and}\  \mathbb{\hat E}\Delta(\phi;(\mX,Y)) +\lambda|\phi|- \lambda J\leq a_n \right\},
\end{align}
where the second line is newly added, and the last line comes from the fact that
\[
\Delta (\phi;(\mX,Y)) :=\Delta(F') (\phi;(\mX,Y))  \leq  \Delta(F) (\phi;(\mX,Y)) =:\bar\Delta (\phi;(\mX,Y)),\quad \text{for all }\phi.
\]
Note that the truncation $T= \max(2,J)>\max(2,\sup_n\mnormSize{}{\phi_{\pi}^{(n)}})$ is a constant that incurs no effects to $\riskF(\bayespif)$ or $\riskF(\phi_{\pi}^{(n)})$. The remaining proof follows a similar line of argument as the bounded case. In particular, we invoke the mean-to-variance relationship for bounded $F'$-loss in~\eqref{eq:second}. The final conclusion follows from the excess bound inequality for $T$-truncated risk (c.f. Lemma~\ref{lem:prepare}).
\end{proof}
...
\]
\end{lem}
\begin{proof}[Proof of Lemma~\label{lem:entropy}]
...
\begin{thm}[Theorem 1 in~\cite{scott2011surrogate}] Let $\riskF(\cdot)$ be $F$-risk defined in~\eqref{eq:riskdef} in the main paper, where $\pi\in[-1,1]$. Define conditional risk 
\[
C_{\pi,F}(\mX,t):=F(t)\mathbb{E}_{Y|\mX}(Y-\pi)^{+}+F(-t)\mathbb{E}_{Y|\mX}(Y-\pi)^{-},
\]
...
\[
\psi\left( \risk(\phi)-\inf_{\text{all }\phi}\risk(\phi)\right)\leq \psi \riskF(\phi)-\inf_{\text{all }\psi}\riskF(\phi).
\]
\end{thm}
