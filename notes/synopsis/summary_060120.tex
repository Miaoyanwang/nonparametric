...
\date{}
\institute{
Joint work with Miaoyan Wang, Lexin Li, Helen Zhang\\
...
%{\it A coincidence or something fundamental?} 
%{\color{blue} May be because of rank theorem: r columns are enough to explain structure of  matrix X, by the same way r rows are enough for information of the matrix}
\end{itemize}
\end{frame}
...
\begin{table}[h]
    ...
    \hline Error (SMM) & 0.4827&0.1903&0.1020&{\color{red}0.0663}\\
...
\hline
SMM & 0.95 & 0.95 & 1.0 & 0.75 & 0.9 & {\color{red}0.91}\\
\hline
\end{tabular}}
\caption{\scriptsize correct classification rate in linearly separable data case}
\label{tab: mcr}
\end{table}
...
\hline
SMM & 0.50 & 0.80 & 0.75 & 0.7 & 0.75 & {\color{red}0.70}\\
\hline
\end{tabular}}
\caption{\scriptsize correct classification rate in linearly non separable data case}
\end{table}
\end{itemize}
...
\begin{itemize}
\item [Regression] How does SVM-based learning compare to other nonparametric regression, such as local smoothing, K-NN, Neural Networks?\\
\vspace{.4cm}
 $\Rightarrow$ SVM-based learning is essentially a kernel smoothing method. Neighborhood is defined by support vectors (c.f.~\eqref{eq:f}). 
 \vspace{.2cm}
