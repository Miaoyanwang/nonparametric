...
visualization=function(B_est,percent=0.9,window=0.1){
d=dim(B_est)[1];H=dim(B_est)[3]
if(H>1){
score=matrix(0,nrow=d,ncol=d)
for(i in 1:d){
    for(j in 1:d){
score[i,j]=max(abs(movavg(B_est[i,j,],max(ceil(window*H),2),type=c("s"))))
    }
}
score[abs(score)<quantile(abs(score),percent)]=0
}else{
score=res3$B
score[abs(score)<quantile(abs(score),percent)]=0
}
...
#################### Our method ####################
NonparaMatrix=function(X,y,X_new=NULL,r,sparse_r,sparse_c,H=10,lambda=0,rho.ini=0.1,min,max){
    ...
    res=list();
    res$B_est=B_est; res$prob=min+prob_est(pred)*(max-min); res$fitted=pred;
    return(res)
}


### alternating optimization for simutanous low-rank+sparse model
ADMM=function(X,ybar,Weight,Covariate=NULL,r,srow,scol,lambda=0,rho.ini=1){
  ...
  result$fitted=yfit;
  result$B=PQ;
  result$residual=residual[-1];
  result$B_appr=res$coef*average_scale;
  result$rho=rho_list;
